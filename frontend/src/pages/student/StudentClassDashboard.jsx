  ...
  useEffect(() => {
    ...
    const fetchSubject = async () => {
      ...
      try {
        ...
        let url;
        if (id) {
          url = `http://localhost:5000/api/student/subjects/${id}`;
        } else if (codeFromQuery) {
          url = `http://localhost:5000/api/student/subjects?class_code=${encodeURIComponent(codeFromQuery)}`;
        } else if (location.state?.classData?.class_code) {
          url = `http://localhost:5000/api/student/subjects?class_code=${encodeURIComponent(location.state.classData.class_code)}`;
        } else {
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-[#e6f8ff] to-white">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 px-6 sm:px-10 py-10 mt-12 md:ml-24">
