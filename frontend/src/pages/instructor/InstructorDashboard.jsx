...
function DashboardPage() {
  ...
  // username synced from settings/sidebar (localStorage key 'username' or instructor_profile)
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setMessage(res.data.message || "Welcome Instructor!");
  ...
  // listen for profile changes (sidebar/settings) so we can update the displayed username live
  useEffect(() => {
    const handler = (e) => {
      try {
        const payload = e?.detail || JSON.parse(localStorage.getItem('instructor_profile') || '{}');
        if (!payload) return;
        if (payload.name) setUsername(payload.name);
        // also try top-level username key
        const topName = localStorage.getItem('username');
        if (topName) setUsername(topName);
      } catch (err) {
        // ignore
      }
    };
  ...
  const handleCreateClass = async () => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/api/instructor/classes", payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      const refresh = await axios.get("http://localhost:5000/api/instructor/dashboard", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUpdateClass = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/instructor/subjects/${subjectId}`,
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleArchive = async (cls) => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/instructor/archive/${subjectId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-[#E0EAFC] to-[#CFDEF3] transition-all duration-500 select-none">
      ...
      <div className="flex flex-1">
        ...
        >
          ...
          {message && (
            <p className="text-black text-lg mb-2 font-medium tracking-wide text-center">
