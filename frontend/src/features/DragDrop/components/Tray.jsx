// ============================================
// ENHANCED TRAY WITH FIXED POSITIONING
// ============================================
export class Tray {
  constructor(scene, components, options = {}) {
    ...
    // Store original positions for each component
  ...
  show(buildStep = 1) {
    ...
    this.components.forEach((comp, i) => {
      ...
      // Store the FINAL position (after animation completes)
      const finalX = isVertical ? endX : imgX;
      const finalY = isVertical ? imgY : endY;
      
      this.componentPositions.set(img, { 
        x: finalX, 
        y: finalY 
      });
    });
  ...
  resetComponentPosition(obj) {
    // Get the stored original position for this specific component
    const originalPos = this.componentPositions.get(obj);
    ...
    if (originalPos) {
      ...
      // Fallback to old behavior if position not found
      console.warn('Tray: Component position not found, using fallback');
