...
import { isAccessibilityInformation } from '@codingame/monaco-vscode-api/vscode/vs/platform/accessibility/common/accessibility';
import { Iterable } from '@codingame/monaco-vscode-api/vscode/vs/base/common/iterator';
import { ExtensionIdentifier } from '@codingame/monaco-vscode-api/vscode/vs/platform/extensions/common/extensions';
import { asStatusBarItemIdentifier } from '@codingame/monaco-vscode-api/vscode/vs/workbench/api/common/extHostTypes';
...
const statusBarItemSchema = {
    ...
    properties: {
        ...
        tooltip: {
            type: 'string',
            description: ( localize(2548, 'The tooltip text for the entry.'))
        },
        command: {
            type: 'string',
            description: ( localize(2549, 'The command to execute when the status bar entry is clicked.'))
        },
        alignment: {
            type: 'string',
            enum: ['left', 'right'],
            description: ( localize(2550, 'The alignment of the status bar entry.'))
        },
        priority: {
            type: 'number',
            description: ( localize(
...
const statusBarItemsSchema = {
    description: ( localize(2555, "Contributes items to the status bar.")),
    oneOf: [
        statusBarItemSchema,
        {
            type: 'array',
            ...
            for (const entry of extensions) {
                ...
                for (const candidate of Iterable.wrap(value)) {
                    if (!isUserFriendlyStatusItemEntry(candidate)) {
                        collector.error(( localize(2556, "Invalid status bar item contribution.")));
                        continue;
                    }
...
let StatusBarItemsExtensionPointWorkbenchContribution = class StatusBarItemsExtensionPointWorkbenchContribution {
    static { this.ID = 'workbench.contrib.statusBarItemsExtensionPoint'; }
    constructor(instantiationService) {
        this.instantiationService = instantiationService;
        this.instantiationService.createInstance(StatusBarItemsExtensionPoint);
    }
};
StatusBarItemsExtensionPointWorkbenchContribution = ( __decorate([
