...
class ConfigureDisplayLanguageAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(8554, "Configure Display Language")),
            menu: {
    ...
    async run(accessor) {
        ...
        qp.placeholder = ( localize(8556, "Select Display Language"));
        if (installedLanguages?.length) {
            const items = [{ type: 'separator', label: ( localize(8557, "Installed")) }];
        ...
        languagePackService.getAvailableLanguages().then(availableLanguages => {
            ...
            if (newLanguages.length) {
                qp.items = [
                    ...qp.items,
                    { type: 'separator', label: ( localize(8558, "Available")) },
    ...
    withMoreInfoButton(items) {
        for (const item of items) {
            if (item.extensionId) {
                item.buttons = [{
                        tooltip: ( localize(8559, "More Info")),
...
class ClearDisplayLanguageAction extends Action2 {
    ...
    static { this.LABEL = ( localize2(8560, "Clear Display Language Preference")); }
    constructor() {
        super({
