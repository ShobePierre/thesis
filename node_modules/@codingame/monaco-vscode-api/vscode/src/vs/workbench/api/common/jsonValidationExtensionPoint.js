...
const configurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(2686, 'Contributes json schema configuration.')),
        type: 'array',
        ...
        configurationExtPoint.setHandler((extensions) => {
            for (const extension of extensions) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(2689, "'configuration.jsonValidation' must be a array")));
                    return;
                }
                extensionValue.forEach(extension => {
                    ...
                    if (!isString(uri)) {
                        collector.error(( localize(2691, "'configuration.jsonValidation.url' must be a URL or relative path")));
                        return;
                    }
                    if (uri.startsWith('./')) {
                        try {
...
class JSONValidationDataRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [
            ( localize(2695, "File Match")),
            ( localize(2696, "Schema")),
        ];
...
( Registry.as(Extensions.ExtensionFeaturesRegistry)).registerExtensionFeature({
    id: 'jsonValidation',
    label: ( localize(2697, "JSON Validation")),
    access: {
        canToggle: false
    },
