...
import * as runtimeExtensionsEditor from './media/runtimeExtensionsEditor.css';
import Severity$1 from '../../../../base/common/severity.js';

var AbstractRuntimeExtensionsEditor_1;
registerCss(runtimeExtensionsEditor);
    ...
    createEditor(parent) {
        ...
        const renderer = {
            ...
            renderElement: (element, index, data) => {
                ...
                if (activationTimes) {
                    ...
                    if (activationEvent === '*') {
                        title = ( localize(7233, "Activated by {0} on start-up", activationId));
                    }
                        ...
                        title = ( localize(7237, "Activated by {0} after start-up finished", activationId));
                    }
                        ...
                        title = ( localize(7239, "Activated by {1} on {0}", activationEvent, activationId));
                    }
                }
                else {
                    title = ( localize(7240, "Extension is activating..."));
                }
                ...
                if (this._getUnresponsiveProfile(element.description.identifier)) {
                    ...
                    const extensionHostFreezTitle = ( localize(7241, "Extension has caused the extension host to freeze."));
                ...
                if (isNonEmptyArray(element.status.runtimeErrors)) {
                    const el = $('span', undefined, ...renderLabelWithIcons(`$(bug) ${( localize(7242, "{0} uncaught errors", element.status.runtimeErrors.length))}`));
                    data.msgContainer.appendChild(el);
                }
                ...
                for (const feature of features) {
                    ...
                    if (accessData) {
                        ...
                        if (accessData?.accessTimes.length > 0) {
                            ...
                            ))}${accessData.current ? ( localize(7244, ", {0} Requests (Session)", accessData.current.accessTimes.length)) : ''}`);
        ...
        this._list = this._instantiationService.createInstance((WorkbenchList), 'RuntimeExtensions', parent, delegate, [renderer], {
            ...
            accessibilityProvider: new (class {
                getWidgetAriaLabel() {
                    return localize(7246, "Runtime Extensions");
                }
        ...
        this._register(this._list.onContextMenu((e) => {
            ...
            const actions = [];
            actions.push(( new Action('runtimeExtensionsEditor.action.copyId', ( localize(7247, "Copy id ({0})", e.element.description.identifier.value)), undefined, true, () => {
            ...
            if (e.element.marketplaceInfo) {
                actions.push(( new Action('runtimeExtensionsEditor.action.disableWorkspace', ( localize(7248, "Disable (Workspace)")), undefined, true, () => this._extensionsWorkbenchService.setEnablement(e.element.marketplaceInfo, EnablementState.DisabledWorkspace))));
                actions.push(( new Action('runtimeExtensionsEditor.action.disable', ( localize(7249, "Disable")), undefined, true, () => this._extensionsWorkbenchService.setEnablement(e.element.marketplaceInfo, EnablementState.DisabledGlobally))));
            }
            actions.push(( new Separator()));
...
class ShowRuntimeExtensionsAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(7250, "Show Running Extensions")),
...
export { AbstractRuntimeExtensionsEditor, ShowRuntimeExtensionsAction };
