        ...
        for (let i = 0; i < str.length; i++) {
            if (str[i] === '\\') {
                ...
                if (escaped === 'n') {
                    ...
                    throw Error(( localize(10919, "All backslashes in Query string must be escaped (\\\\)")));
                }
            }
            else {
                out += str[i];
...
const serializeSearchResultForEditor = (searchResult, rawIncludePattern, rawExcludePattern, contextLines, labelFormatter, sortOrder, limitHit) => {
    ...
    const filecount = searchResult.fileCount() > 1 ? ( localize(10920, "{0} files", searchResult.fileCount())) : ( localize(10921, "1 file"));
    const resultcount = searchResult.count() > 1 ? ( localize(10922, "{0} results", searchResult.count())) : ( localize(10923, "1 result"));
    const info = [
        searchResult.count()
            ? `${resultcount} - ${filecount}`
            : ( localize(10924, "No Results")),
    ];
    if (limitHit) {
        info.push(( localize(
            10925,
