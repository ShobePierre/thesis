...
let TempFileMergeEditorInputModel = class TempFileMergeEditorInputModel extends EditorModel {
    constructor(model, disposable, result, resultUri, textFileService, dialogService, editorService) {
        ...
        this.altVersionId = observableFromEvent(this, e => this.model.resultTextModel.onDidChangeContent(e), () =>  this.model.resultTextModel.getAlternativeVersionId());
        ...
        if (someDirty) {
            ...
            const buttons = [
                {
                    label: hasUnhandledConflicts ?
                        ( localize(9094, "&&Save With Conflicts")) :
                        ( localize(9095, "&&Save")),
                    run: () => ConfirmResult.SAVE
                },
                {
                    label: ( localize(9096, "Do&&n't Save")),
            ...
            choice = (await this.dialogService.prompt({
                ...
                detail: hasUnhandledConflicts
                    ...
                    : isMany
                        ? ( localize(9099, "The merge results will be lost if you don't save them."))
                        : ( localize(9100, "The merge result will be lost if you don't save it.")),
                buttons,
                cancelButton: {
    ...
    async save(options) {
        ...
        (async () => {
            const { confirmed } = await this.dialogService.confirm({
                message: ( localize(9101, "Do you want to accept the merge result?")),
                detail: ( localize(
                    9102,
                    "This will write the merge result to the original file and close the merge editor."
                )),
                primaryButton: ( localize(9103, '&&Accept Merge'))
            });
            if (confirmed) {
                await this.accept();
...
let WorkspaceMergeEditorModeFactory = class WorkspaceMergeEditorModeFactory {
    ...
    static { this.FILE_SAVED_SOURCE = SaveSourceRegistry.registerSource('merge-editor.source', ( localize(9104, "Before Resolving Conflicts In Merge Editor"))); }
    async createInputModel(args) {
        const store = ( new DisposableStore());
        ...
        if (someDirty) {
            ...
            const { result } = await this._dialogService.prompt({
                ...
                detail: someUnhandledConflicts ?
                    ...
                    : isMany
                        ? ( localize(9109, "Your changes will be lost if you don't save them."))
                        : ( localize(9110, "Your changes will be lost if you don't save them.")),
                buttons: [
                    {
                        label: someUnhandledConflicts
                            ? ( localize(9111, '&&Save with Conflicts'))
                            : ( localize(9112, '&&Save')),
                        run: () => ConfirmResult.SAVE
                    },
                    {
                        label: ( localize(9113, "Do&&n't Save")),
                        run: () => ConfirmResult.DONT_SAVE
                    }
                ],
                cancelButton: {
            ...
            const { confirmed, checkboxChecked } = await this._dialogService.confirm({
                message: isMany
                    ? ( localize(9114, 'Do you want to close {0} merge editors?', inputModels.length))
                    : ( localize(
                    9115,
                ...
                detail: someUnhandledConflicts ?
                    isMany
                        ? ( localize(9116, "The files contain unhandled conflicts."))
                        : ( localize(9117, "The file contains unhandled conflicts."))
                    : undefined,
                primaryButton: someUnhandledConflicts
                    ? ( localize(9118, '&&Close with Conflicts'))
                    : ( localize(9119, '&&Close')),
                checkbox: { label: ( localize(9120, "Do not ask me again")) }
            });
            if (checkboxChecked) {
