...
let ExtensionsListView = class ExtensionsListView extends AbstractExtensionsListView {
    ...
    static { this.RECENT_UPDATE_DURATION = 7 * 24 * 60 * 60 * 1000; }
    ...
    async queryLocal(query, options) {
        ...
        return {
            ...
            message: description ? { text: description, severity: Severity$1.Info } : undefined,
    ...
    filterEnabledExtensions(local, runningExtensions, query, options, includeBuiltin) {
        ...
        const result = local
            .sort((e1, e2) => e1.displayName.localeCompare(e2.displayName))
            .filter(e => ( runningExtensions.some(
            r => areSameExtensions({ id: r.identifier.value, uuid: r.uuid }, e.identifier)
        ))
            && (e.name.toLowerCase().indexOf(value) > -1 || e.displayName.toLowerCase().indexOf(value) > -1)
            && this.filterExtensionByCategory(e, includedCategories, excludedCategories));
        return this.sortExtensions(result, options);
    ...
    filterExtensionsByFeature(local, query) {
        ...
        try {
            ...
            return {
                ...
                description: ( localize(7723, "Extensions using {0} in the last 30 days", feature.label))
            };
        }
        finally {
            renderer?.dispose();
    ...
    async queryGallery(query, options, token) {
        ...
        try {
            ...
            if (localExtensions.length) {
                ...
                return { model: ( new PagedModel(localExtensions)), disposables: ( new DisposableStore()), message: { text: ( localize(7724, "{0} Showing local extensions.", message.text)), severity: message.severity } };
            }
            throw error;
        }
    }
    ...
    async getOtherRecommendationsModel(query, options, token) {
        ...
        const result = coalesce(( otherRecommendations.map(
            id => installableRecommendations.find(i => areSameExtensions(i.identifier, { id }))
        )));
        return ( new PagedModel(result));
    }
        ...
        if (this.bodyTemplate) {
            ...
            if (this.isBodyVisible()) {
                if (message) {
                    ...
                    this.bodyTemplate.messageBox.textContent = ( localize(7725, "No extensions found."));
                }
    ...
    getMessage(error) {
        if (this.isOfflineError(error)) {
            ...
            return { text: ( localize(7727, "Error while fetching extensions. {0}", getErrorMessage(error))), severity: Severity$1.Error };
        }
    }
...
class WorkspaceRecommendedExtensionsView extends ExtensionsListView {
    ...
    async getInstallableWorkspaceRecommendations() {
        const installed = (await this.extensionsWorkbenchService.queryLocal())
            .filter(l => l.enablementState !== EnablementState.DisabledByExtensionKind);
        const recommendations = (await this.getWorkspaceRecommendations())
    ...
    async installWorkspaceRecommendations() {
        ...
        if (installableRecommendations.length) {
            ...
            this.notificationService.notify({
                ...
                message: ( localize(7728, "There are no extensions to install."))
            });
        }
    }
}
