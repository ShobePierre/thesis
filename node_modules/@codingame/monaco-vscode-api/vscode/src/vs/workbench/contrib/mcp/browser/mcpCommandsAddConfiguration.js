...
const AssistedTypes = {
    [AddConfigurationType.NpmPackage]: {
        title: ( localize(8751, "Enter NPM Package Name")),
        placeholder: ( localize(8752, "Package name (e.g., @org/package)")),
        pickLabel: ( localize(8753, "NPM Package")),
        pickDescription: ( localize(8754, "Install from an NPM package name")),
        enabledConfigKey: null,
    },
    [AddConfigurationType.PipPackage]: {
        title: ( localize(8755, "Enter Pip Package Name")),
        placeholder: ( localize(8756, "Package name (e.g., package-name)")),
        pickLabel: ( localize(8757, "Pip Package")),
        pickDescription: ( localize(8758, "Install from a Pip package name")),
        enabledConfigKey: null,
    },
    [AddConfigurationType.NuGetPackage]: {
        title: ( localize(8759, "Enter NuGet Package Name")),
        placeholder: ( localize(8760, "Package name (e.g., Package.Name)")),
        pickLabel: ( localize(8761, "NuGet Package")),
        pickDescription: ( localize(8762, "Install from a NuGet package name")),
        enabledConfigKey: 'chat.mcp.assisted.nuget.enabled',
    },
    [AddConfigurationType.DockerImage]: {
        title: ( localize(8763, "Enter Docker Image Name")),
        placeholder: ( localize(8764, "Image name (e.g., mcp/imagename)")),
        pickLabel: ( localize(8765, "Docker Image")),
        pickDescription: ( localize(8766, "Install from a Docker image")),
        enabledConfigKey: null,
    },
};
...
let McpAddConfigurationCommand = class McpAddConfigurationCommand {
    ...
    async getServerType() {
        const items = [
            { kind: AddConfigurationType.Stdio, label: ( localize(8767, "Command (stdio)")), description: ( localize(8768, "Run a local command that implements the MCP protocol")) },
            { kind: AddConfigurationType.HTTP, label: ( localize(8769, "HTTP (HTTP or Server-Sent Events)")), description: ( localize(8770, "Connect to a remote HTTP server that implements the MCP protocol")) }
        ];
        let aiSupported;
        try {
        ...
        if (aiSupported) {
            items.unshift({ type: 'separator', label: ( localize(8771, "Manual Install")) });
            ...
            items.push({ type: 'separator', label: ( localize(8772, "Model-Assisted")) }, ...elligableTypes);
        }
        ...
        if (discovery && typeof discovery === 'object' && ( allDiscoverySources.some(d => !discovery[d]))) {
            items.push({
                kind: 'discovery',
                label: ( localize(8773, "Add from another application...")),
            });
        }
        items.push({
            kind: 'browse',
            label: ( localize(8774, "Browse MCP Servers...")),
        });
        const result = await this._quickInputService.pick(items, {
            placeHolder: ( localize(8775, "Choose the type of MCP server to add")),
        });
    ...
    async getStdioConfig() {
        const command = await this._quickInputService.input({
            title: ( localize(8776, "Enter Command")),
            placeHolder: ( localize(8777, "Command to run (with optional arguments)")),
    ...
    async getSSEConfig() {
        const url = await this._quickInputService.input({
            title: ( localize(8778, "Enter Server URL")),
            placeHolder: ( localize(8779, "URL of the MCP server (e.g., http://localhost:3000)")),
            ignoreFocusLost: true,
        });
        if (!url) {
            return undefined;
    ...
    async getServerId(suggestion = `my-mcp-server-${generateUuid().split('-')[0]}`) {
        const id = await this._quickInputService.input({
            title: ( localize(8780, "Enter Server ID")),
            placeHolder: ( localize(8781, "Unique identifier for this server")),
            value: suggestion,
            ignoreFocusLost: true,
        });
        return id;
    ...
    async getConfigurationTarget() {
        const options = [
            { target: ConfigurationTarget.USER_LOCAL, label: ( localize(8782, "Global")), description: ( localize(8783, "Available in all workspaces, runs locally")) }
        ];
        ...
        if (raLabel) {
            options.push({ target: ConfigurationTarget.USER_REMOTE, label: ( localize(8784, "Remote")), description: ( localize(8785, "Available on this remote machine, runs on {0}", raLabel)) });
        }
        ...
        if (workbenchState !== WorkbenchState.EMPTY) {
            ...
            if (this._environmentService.remoteAuthority) {
                options.push({ target, label: ( localize(8786, "Workspace")), description: ( localize(8787, "Available in this workspace, runs on {0}", raLabel)) });
            }
            else {
                options.push({ target, label: ( localize(8786, "Workspace")), description: ( localize(8788, "Available in this workspace, runs locally")) });
            }
        }
        if (options.length === 1) {
        ...
        const targetPick = await this._quickInputService.pick(options, {
            title: ( localize(8789, "Add MCP Server")),
            placeHolder: ( localize(8790, "Select the configuration target"))
        });
    ...
    async getAssistedConfig(type) {
        ...
        loadingQuickPick.title = ( localize(8791, "Loading package details..."));
            ...
            if (!result || result.state === 'error') {
                ...
                if (result?.helpUri) {
                    items.push({
                        ...
                        label: result.helpUriLabel ?? ( localize(8792, 'Open help URL')),
                        helpUri: ( URI.parse(result.helpUri)),
                    });
                }
                items.push({ id: LoadAction.Retry, label: ( localize(8793, 'Try a different package')) }, { id: LoadAction.Cancel, label: ( localize(8794, 'Cancel')) });
                ...
                loadingQuickPick.items = [
                    { id: LoadAction.Allow, label: ( localize(8796, "Allow")) },
                    { id: LoadAction.Cancel, label: ( localize(8794, 'Cancel')) }
                ];
            }
    ...
    async pickForUrlHandler(resource, showIsPrimary = false) {
        ...
        const placeHolder = ( localize(8797, 'Install MCP server {0}', name));
        const items = [
            { id: 'install', label: ( localize(8798, 'Install Server')) },
            { id: 'show', label: ( localize(8799, 'Show Configuration', name)) },
            { id: 'rename', label: ( localize(8800, 'Rename "{0}"', name)) },
            { id: 'cancel', label: ( localize(8794, 'Cancel')) },
        ];
        if (showIsPrimary) {
        ...
        switch (pick?.id) {
            ...
            case 'install':
                ...
                try {
                    ...
                    this._notificationService.error(( localize(8801, 'Error installing MCP server {0}: {1}', name, e.message)));
                    await this._editorService.openEditor({ resource });
                }
                break;
            case 'rename': {
                const newName = await this._quickInputService.input({ placeHolder: ( localize(8802, 'Enter new name')), value: name });
                if (newName) {
