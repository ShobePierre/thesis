...
let RuntimeStatusMarkdownRenderer = class RuntimeStatusMarkdownRenderer extends Disposable {
    ...
    createElement(manifest, disposables) {
        ...
        if (( this.extensionService.extensions.some(extension => ExtensionIdentifier.equals(extension.identifier, extensionId)))) {
            ...
            data.appendMarkdown(`### ${( localize(7297, "Activation"))}\n\n`);
        ...
        if (status.runtimeErrors.length || status.messages.length) {
            ...
            if (status.runtimeErrors.length) {
                data.appendMarkdown(`\n ### ${( localize(7300, "Uncaught Errors ({0})", status.runtimeErrors.length))}\n`);
                for (const error of status.runtimeErrors) {
                    data.appendMarkdown(`$(${Codicon.error.id})&nbsp;${getErrorMessage(error)}\n\n`);
                }
            }
            if (status.messages.length) {
                data.appendMarkdown(`\n ### ${( localize(7301, "Messages ({0})", status.messages.length))}\n`);
...
const runtimeStatusFeature = {
    ...
    label: ( localize(7302, "Runtime Status")),
    access: {
        canToggle: false
    },
...
let ExtensionFeaturesTab = class ExtensionFeaturesTab extends Themable {
    ...
    create() {
        ...
        if (features.length === 0) {
            append($('.no-features'), this.domNode).textContent = ( localize(7303, "No features contributed."));
            return;
        }
    ...
    createFeaturesList(container) {
        ...
        const list = this.instantiationService.createInstance(WorkbenchList, 'ExtensionFeaturesList', append(container, $('.features-list-wrapper')), delegate, [renderer], {
            ...
            accessibilityProvider: {
                ...
                getWidgetAriaLabel() {
                    return localize(7304, "Extension Features");
                }
            },
...
let ExtensionFeatureItemRenderer = class ExtensionFeatureItemRenderer {
    ...
    renderTemplate(container) {
        ...
        disabledElement.textContent = ( localize(7305, "No Access"));
...
let ExtensionFeatureView = class ExtensionFeatureView extends Disposable {
    ...
    create(content) {
        ...
        if (this.feature.access.canToggle) {
            ...
            this._register(button.onDidClick(async () => {
                ...
                const confirmationResult = await this.dialogService.confirm({
                    title: ( localize(7306, "Enable '{0}' Feature", this.feature.label)),
                    message: enabled
                        ? ( localize(
                        7307,
                    ...
                    custom: true,
                    primaryButton: enabled ? ( localize(7309, "Revoke Access")) : ( localize(7310, "Allow Access")),
                    cancelButton: ( localize(7311, "Cancel")),
                });
        ...
        if (accessData?.current?.status) {
            append(bodyContent, $('.feature-status', undefined, $(`span${ThemeIcon.asCSSSelector(accessData.current.status.severity === Severity$1.Error ? errorIcon : accessData.current.status.severity === Severity$1.Warning ? warningIcon : infoIcon)}`, undefined), $('span', undefined, accessData.current.status.message)));
        }
    ...
    updateButtonLabel(button) {
        button.label = this.extensionFeaturesManagementService.isEnabled(this.extensionId, this.feature.id) ? ( localize(7309, "Revoke Access")) : ( localize(7312, "Allow Access"));
    }
