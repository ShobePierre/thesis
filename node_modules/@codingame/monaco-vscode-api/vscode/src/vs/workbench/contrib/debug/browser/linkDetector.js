...
let LinkDetector = class LinkDetector {
    ...
    decorateLink(link, uri, fulltext, hoverBehavior, onClick) {
        ...
        const followLink = uri && this.tunnelService.canTunnel(uri) ? ( localize(6915, "follow link using forwarded port")) : ( localize(6916, "follow link"));
        const title = link.ariaLabel = fulltext
            ? (isMacintosh ? ( localize(6917, "Cmd + click to {0}\n{1}", followLink, fulltext)) : ( localize(6918, "Ctrl + click to {0}\n{1}", followLink, fulltext)))
            : (isMacintosh ? ( localize(6919, "Cmd + click to {0}", followLink)) : ( localize(6920, "Ctrl + click to {0}", followLink)));
