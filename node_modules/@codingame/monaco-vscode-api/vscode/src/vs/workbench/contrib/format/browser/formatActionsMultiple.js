...
let DefaultFormatter = class DefaultFormatter extends Disposable {
    ...
    async _updateConfigValues() {
        ...
        DefaultFormatter_1.extensionItemLabels.push(( localize(8248, 'None')));
        DefaultFormatter_1.extensionDescriptions.push(( localize(8249, "None")));
    ...
    async _selectFormatter(formatter, document, mode, kind) {
        ...
        if (mode !== FormattingMode.Silent) {
            const { confirmed } = await this._dialogService.confirm({
                message: ( localize(8254, "Configure Default Formatter")),
                detail: formatterOrMessage,
                primaryButton: ( localize(8255, "&&Configure..."))
            });
            if (confirmed) {
            ...
            this._notificationService.prompt(Severity$1.Info, formatterOrMessage, [{ label: ( localize(8256, "Configure...")), run: () => this._pickAndPersistDefaultFormatter(formatter, document) }], { priority: NotificationPriority.SILENT });
        }
        return undefined;
    }
    ...
    _updateStatus() {
        ...
        this._analyzeFormatter(FormattingKind.File, formatter, document).then(result => {
            ...
            const command = { id: `formatter/configure/dfl/${generateUuid()}`, title: ( localize(8258, "Configure...")) };
...
async function showFormatterPick(accessor, model, formatters) {
    ...
    const picks = ( formatters.map((provider, index) => {
        ...
        const pick = {
            ...
            description: isDefault ? ( localize(8262, "(default)")) : undefined,
        };
        if (isDefault) {
    ...
    const configurePick = {
        label: ( localize(8263, "Configure Default Formatter..."))
    };
    const pick = await quickPickService.pick([...picks, { type: 'separator' }, configurePick], {
        placeHolder: ( localize(8264, "Select a formatter")),
...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize(8265, "Format Document With...")),
...
registerEditorAction(class FormatSelectionMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize(8266, "Format Selection With...")),
            alias: 'Format Code...',
