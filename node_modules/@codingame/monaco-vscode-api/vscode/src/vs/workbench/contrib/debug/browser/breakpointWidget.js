...
let BreakpointWidget = class BreakpointWidget extends ZoneWidget {
    ...
    _fillContainer(container) {
        ...
        const selectBox = this.store.add(( new SelectBox([
            { text: ( localize(6462, "Expression")) },
            { text: ( localize(6463, "Hit Count")) },
            { text: ( localize(6464, "Log Message")) },
            { text: ( localize(6465, "Wait for Breakpoint")) },
        ], this.context, this.contextViewService, defaultSelectBoxStyles, { ariaLabel: ( localize(6466, 'Breakpoint Type')), useCustomDrawn: !hasNativeContextMenu(this._configurationService) })));
    ...
    createModesInput(container) {
        ...
        const sb = this.selectModeBox = ( new SelectBox([
            { text: ( localize(6467, 'Mode')), isDisabled: true },
    ...
    createTriggerBreakpointInput(container) {
        ...
        const breakpointOptions = [
            { text: ( localize(6468, 'None')), isDisabled: true },
            ...( breakpoints.map(bp => ({
                text: `${this.labelService.getUriLabel(bp.uri, { relative: true })}: ${bp.lineNumber}`,
                description: ( localize(6469, 'Loading...'))
            }))),
        ];
        ...
        for (const [i, bp] of breakpoints.entries()) {
            this.textModelService.createModelReference(bp.uri).then(ref => {
                ...
                breakpointOptions[i + 1].description = ( localize(6470, 'Could not load source.'));
            });
        }
        const selectBreakpointBox = this.selectBreakpointBox = this.store.add(( new SelectBox(
            ...
            { ariaLabel: ( localize(6471, 'Select breakpoint')), useCustomDrawn: !hasNativeContextMenu(this._configurationService) }
        )));
        ...
        closeButton.label = ( localize(6472, "OK"));
