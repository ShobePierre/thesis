            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([( new ResourceFileEdit(
                        ...
                        undoLabel: ( localize(8048, "Overwrite {0}", entry.name)),
                        progressLabel: ( localize(8049, "Overwriting {0}", entry.name)),
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
    ...
    async doImport(target, source, targetWindow, token) {
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            const buttons = [
                {
                    label: folders.length > 1 ?
                        ( localize(8053, "&&Copy Folders")) :
                        ( localize(8054, "&&Copy Folder")),
            ...
            if (( folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0))) {
                buttons.unshift({
                    label: folders.length > 1 ?
                        ( localize(8055, "&&Add Folders to Workspace")) :
                        ( localize(8056, "&&Add Folder to Workspace")),
                ...
                message = folders.length > 1 ?
                    ( localize(8059, "Are you sure to want to copy folders?")) :
                    ( localize(8060, "Are you sure to want to copy '{0}'?", basename(folders[0].uri)));
            }
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                this.notificationService.error(inaccessibleFileCount > 1 ? ( localize(8061, "Some or all of the dropped files could not be accessed for import.")) : ( localize(8062, "The dropped file could not be accessed for import.")));
            }
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ?
                    ( localize(8063, "Import {0}", basename(resourcesFiltered[0]))) :
                    ( localize(8064, "Import {0} resources", resourcesFiltered.length)),
                progressLabel: resourcesFiltered.length === 1 ?
                    ( localize(8065, "Copying {0}", basename(resourcesFiltered[0]))) :
                    ( localize(8066, "Copying {0} resources", resourcesFiltered.length)),
...
let FileDownload = class FileDownload {
    ...
    download(source) {
        ...
        const downloadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: isWeb,
            title: ( localize(8067, "Downloading"))
    ...
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: ( localize(8070, "Download")),
            title: ( localize(8071, "Choose Where to Download")),
            defaultUri
        });
        if (destination) {
            ...
            await this.explorerService.applyBulkEdit([( new ResourceFileEdit(explorerItem.resource, destination, { overwrite: true, copy: true }))], {
                undoLabel: ( localize(8072, "Download {0}", explorerItem.name)),
                progressLabel: ( localize(8073, "Downloading {0}", explorerItem.name)),
                progressLocation: ProgressLocation.Window
            });
        }
        else {
...
function getFileOverwriteConfirm(name) {
    return {
        ...
        detail: ( localize(8075, "This action is irreversible!")),
        primaryButton: ( localize(8076, "&&Replace")),
        type: 'warning'
    };
}
function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            ...
            detail: getFileNamesMessage(files) + '\n' + ( localize(8075, "This action is irreversible!")),
            primaryButton: ( localize(8076, "&&Replace")),
            type: 'warning'
        };
    }
