...
(function (ChatContextKeys) {
    ...
    ChatContextKeys.responseDetectedAgentCommand = ( new RawContextKey(
        ...
        false,
        { type: 'boolean', description: ( localize(5877, "When the agent or command was automatically detected")) }
    ));
    ChatContextKeys.responseSupportsIssueReporting = ( new RawContextKey(
        'chatResponseSupportsIssueReporting',
        false,
        { type: 'boolean', description: ( localize(5878, "True when the current chat response supports issue reporting.")) }
    ));
    ChatContextKeys.responseIsFiltered = ( new RawContextKey(
        'chatSessionResponseFiltered',
        false,
        { type: 'boolean', description: ( localize(5879, "True when the chat response was filtered out by the server.")) }
    ));
    ChatContextKeys.responseHasError = ( new RawContextKey(
        'chatSessionResponseError',
        false,
        { type: 'boolean', description: ( localize(5880, "True when the chat response resulted in an error.")) }
    ));
    ChatContextKeys.requestInProgress = ( new RawContextKey(
        'chatSessionRequestInProgress',
        false,
        { type: 'boolean', description: ( localize(5881, "True when the current request is still in progress.")) }
    ));
    ChatContextKeys.currentlyEditing = ( new RawContextKey(
        'chatSessionCurrentlyEditing',
        false,
        { type: 'boolean', description: ( localize(5882, "True when the current request is being edited.")) }
    ));
    ChatContextKeys.currentlyEditingInput = ( new RawContextKey(
        'chatSessionCurrentlyEditingInput',
        false,
        { type: 'boolean', description: ( localize(5883, "True when the current request input at the bottom is being edited.")) }
    ));
    ChatContextKeys.isResponse = ( new RawContextKey('chatResponse', false, { type: 'boolean', description: ( localize(5884, "The chat item is a response.")) }));
    ChatContextKeys.isRequest = ( new RawContextKey('chatRequest', false, { type: 'boolean', description: ( localize(5885, "The chat item is a request")) }));
    ChatContextKeys.itemId = ( new RawContextKey('chatItemId', '', { type: 'string', description: ( localize(5886, "The id of the chat item.")) }));
    ChatContextKeys.lastItemId = ( new RawContextKey('chatLastItemId', [], { type: 'string', description: ( localize(5887, "The id of the last chat item.")) }));
    ChatContextKeys.editApplied = ( new RawContextKey('chatEditApplied', false, { type: 'boolean', description: ( localize(5888, "True when the chat text edits have been applied.")) }));
    ChatContextKeys.inputHasText = ( new RawContextKey('chatInputHasText', false, { type: 'boolean', description: ( localize(5889, "True when the chat input has text.")) }));
    ChatContextKeys.inputHasFocus = ( new RawContextKey('chatInputHasFocus', false, { type: 'boolean', description: ( localize(5890, "True when the chat input has focus.")) }));
    ChatContextKeys.inChatInput = ( new RawContextKey('inChatInput', false, { type: 'boolean', description: ( localize(5891, "True when focus is in the chat input, false otherwise.")) }));
    ChatContextKeys.inChatSession = ( new RawContextKey('inChat', false, { type: 'boolean', description: ( localize(5892, "True when focus is in the chat widget, false otherwise.")) }));
    ChatContextKeys.inChatEditor = ( new RawContextKey('inChatEditor', false, { type: 'boolean', description: ( localize(5893, "Whether focus is in a chat editor.")) }));
    ChatContextKeys.inChatTerminalToolOutput = ( new RawContextKey(
        'inChatTerminalToolOutput',
        false,
        { type: 'boolean', description: ( localize(5894, "True when focus is in the chat terminal output region.")) }
    ));
    ChatContextKeys.chatModeKind = ( new RawContextKey(
        'chatAgentKind',
        ChatModeKind.Ask,
        { type: 'string', description: ( localize(5895, "The 'kind' of the current agent.")) }
    ));
    ChatContextKeys.chatToolCount = ( new RawContextKey('chatToolCount', 0, { type: 'number', description: ( localize(5896, "The number of tools available in the current agent.")) }));
    ChatContextKeys.chatToolGroupingThreshold = ( new RawContextKey(
        'chat.toolGroupingThreshold',
        0,
        { type: 'number', description: ( localize(5897, "The number of tools at which we start doing virtual grouping.")) }
    ));
    ...
    ChatContextKeys.lockedToCodingAgent = ( new RawContextKey(
        'lockedToCodingAgent',
        false,
        { type: 'boolean', description: ( localize(5899, "True when the chat widget is locked to the coding agent session.")) }
    ));
    ChatContextKeys.agentSupportsAttachments = ( new RawContextKey(
        'agentSupportsAttachments',
        false,
        { type: 'boolean', description: ( localize(5900, "True when the chat agent supports attachments.")) }
    ));
    ChatContextKeys.withinEditSessionDiff = ( new RawContextKey(
        'withinEditSessionDiff',
        false,
        { type: 'boolean', description: ( localize(5901, "True when the chat widget dispatches to the edit session chat.")) }
    ));
    ChatContextKeys.filePartOfEditSession = ( new RawContextKey(
        'filePartOfEditSession',
        false,
        { type: 'boolean', description: ( localize(5902, "True when the chat widget is within a file with an edit session.")) }
    ));
    ...
    ChatContextKeys.panelParticipantRegistered = ( new RawContextKey(
        ...
        false,
        { type: 'boolean', description: ( localize(5904, "True when a default chat participant is registered for the panel.")) }
    ));
    ...
    ChatContextKeys.languageModelsAreUserSelectable = ( new RawContextKey(
        ...
        false,
        { type: 'boolean', description: ( localize(5907, "True when the chat model can be selected manually by the user.")) }
    ));
    ...
    ChatContextKeys.inQuickChat = ( new RawContextKey('quickChatHasFocus', false, { type: 'boolean', description: ( localize(5910, "True when the quick chat UI has focus, false otherwise.")) }));
    ChatContextKeys.hasFileAttachments = ( new RawContextKey(
        'chatHasFileAttachments',
        false,
        { type: 'boolean', description: ( localize(5911, "True when the chat has file attachments.")) }
    ));
    ChatContextKeys.chatSessionIsEmpty = ( new RawContextKey('chatSessionIsEmpty', true, { type: 'boolean', description: ( localize(5912, "True when the current chat session has no requests.")) }));
    ChatContextKeys.remoteJobCreating = ( new RawContextKey(
        'chatRemoteJobCreating',
        false,
        { type: 'boolean', description: ( localize(5913, "True when a remote coding agent job is being created.")) }
    ));
    ChatContextKeys.hasRemoteCodingAgent = ( new RawContextKey('hasRemoteCodingAgent', false, ( localize(5914, "Whether any remote coding agent is available"))));
    ...
    ChatContextKeys.skipChatRequestInProgressMessage = ( new RawContextKey(
        ...
        false,
        { type: 'boolean', description: ( localize(5916, "True when the chat request in progress message should be skipped.")) }
    ));
    ...
    ChatContextKeys.Editing = {
        hasToolConfirmation: ( new RawContextKey(
            ...
            false,
            { type: 'boolean', description: ( localize(5917, "True when a tool confirmation is present.")) }
        )),
        hasElicitationRequest: ( new RawContextKey(
            'chatHasElicitationRequest',
            false,
            { type: 'boolean', description: ( localize(5918, "True when a chat elicitation request is pending.")) }
        )),
    };
    ChatContextKeys.Tools = {
        toolsCount: ( new RawContextKey('toolsCount', 0, { type: 'number', description: ( localize(5919, "The count of tools available in the chat.")) }))
    };
    ChatContextKeys.Modes = {
        hasCustomChatModes: ( new RawContextKey(
            'chatHasCustomAgents',
            false,
            { type: 'boolean', description: ( localize(5920, "True when the chat has custom agents available.")) }
        )),
        agentModeDisabledByPolicy: ( new RawContextKey(
            'chatAgentModeDisabledByPolicy',
            false,
            { type: 'boolean', description: ( localize(5921, "True when agent mode is disabled by organization policy.")) }
        )),
    };
    ChatContextKeys.panelLocation = ( new RawContextKey(
        'chatPanelLocation',
        undefined,
        { type: 'number', description: ( localize(5922, "The location of the chat panel.")) }
    ));
    ChatContextKeys.isCombinedAgentSessionsViewer = ( new RawContextKey(
        'chatIsCombinedSessionViewer',
        false,
        { type: 'boolean', description: ( localize(5923, "True when the chat session viewer uses the new combined style.")) }
    ));
    ...
    ChatContextKeys.agentSessionsViewerOrientation = ( new RawContextKey(
        ...
        undefined,
        { type: 'number', description: ( localize(5925, "Orientation of the agent sessions view in the chat view.")) }
    ));
    ChatContextKeys.agentSessionsViewerPosition = ( new RawContextKey(
        'agentSessionsViewerPosition',
        undefined,
        { type: 'number', description: ( localize(5926, "Position of the agent sessions view in the chat view.")) }
    ));
    ChatContextKeys.agentSessionType = ( new RawContextKey('chatSessionType', '', { type: 'string', description: ( localize(5927, "The type of the current agent session item.")) }));
    ChatContextKeys.hasAgentSessionChanges = ( new RawContextKey(
        'agentSessionHasChanges',
        false,
        { type: 'boolean', description: ( localize(5928, "True when the current agent session item has changes.")) }
    ));
    ChatContextKeys.isArchivedAgentSession = ( new RawContextKey(
        'agentSessionIsArchived',
        false,
        { type: 'boolean', description: ( localize(5929, "True when the agent session item is archived.")) }
    ));
    ChatContextKeys.isReadAgentSession = ( new RawContextKey(
        'agentSessionIsRead',
        false,
        { type: 'boolean', description: ( localize(5930, "True when the agent session item is read.")) }
    ));
    ChatContextKeys.isActiveAgentSession = ( new RawContextKey(
        'agentSessionIsActive',
        false,
        { type: 'boolean', description: ( localize(5931, "True when the agent session is currently active (not deletable).")) }
    ));
    ChatContextKeys.isKatexMathElement = ( new RawContextKey(
        'chatIsKatexMathElement',
        false,
        { type: 'boolean', description: ( localize(5932, "True when focusing a KaTeX math element.")) }
    ));
