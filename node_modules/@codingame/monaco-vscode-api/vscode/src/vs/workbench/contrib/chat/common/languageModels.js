...
const languageModelChatProviderType = {
    ...
    properties: {
        vendor: {
            type: 'string',
            description: ( localize(5957, "A globally unique vendor of language model chat provider."))
        },
        displayName: {
            type: 'string',
            description: ( localize(5958, "The display name of the language model chat provider."))
        },
        managementCommand: {
            type: 'string',
...
const languageModelChatProviderExtensionPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(5961, "Contribute language model chat providers of a specific vendor.")),
        oneOf: [
            ...
            for (const extension of extensions) {
                for (const item of Iterable.wrap(extension.value)) {
                    ...
                    if (isFalsyOrWhitespace(item.vendor)) {
                        extension.collector.error(( localize(5963, "The vendor field cannot be empty.")));
                        continue;
                    }
                    if (item.vendor.trim() !== item.vendor) {
                        extension.collector.error(( localize(5964, "The vendor field cannot start or end with whitespace.")));
                        continue;
                    }
