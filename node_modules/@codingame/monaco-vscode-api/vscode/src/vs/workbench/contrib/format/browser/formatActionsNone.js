...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(8267, "Format Document")),
    ...
    async run(accessor, editor) {
        ...
        if (formatterCount > 1) {
            ...
            notificationService.warn(( localize(8268, "This file cannot be formatted because it is too large")));
        }
        else {
            const langName = model.getLanguageId();
            const message = ( localize(8269, "There is no formatter for '{0}' files installed.", langName));
            const { confirmed } = await dialogService.confirm({
                message,
                primaryButton: ( localize(8270, "&&Install Formatter..."))
            });
            if (confirmed) {
