...
function getTerminalDecorationHoverContent(command, hoverMessage, showCommandActions) {
    let hoverContent = showCommandActions ? `${( localize(11663, "Show Command Actions"))}\n\n---\n\n` : '';
    if (!command) {
        if (hoverMessage) {
        ...
        if (isNumber(command.duration)) {
            ...
            if (command.exitCode) {
                if (command.exitCode === -1) {
                    hoverContent += ( localize(11667, 'Command executed {0} and failed', fromNow(command.timestamp, true)));
                }
                else {
                    hoverContent += ( localize(
                        11668,
                ...
                hoverContent += ( localize(11669, 'Command executed {0} now'));
            }
        }
    }
    return hoverContent;
...
const unknownText = ( localize(11670, 'Unknown'));
const runningText = ( localize(11671, 'Running'));
function getTerminalCommandDecorationTooltip(command, storedState) {
    ...
    if (typeof duration === 'number') {
        ...
        if (exitCode) {
            ...
            hoverContent += ( localize(11666, 'Command executed {0} and took {1}', fromNowText, durationText));
        }
    }
    else {
        if (exitCode) {
            if (exitCode === -1) {
                hoverContent += ( localize(11667, 'Command executed {0} and failed', fromNowText));
            }
            else {
                hoverContent += ( localize(
                    11668,
            ...
            hoverContent += ( localize(11672, 'Command executed {0} ', fromNowText));
        }
    }
    return hoverContent;
}
