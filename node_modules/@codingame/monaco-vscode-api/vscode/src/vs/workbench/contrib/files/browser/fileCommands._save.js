...
async function doSaveEditors(accessor, editors, options) {
    ...
    try {
        ...
        if (!isCancellationError(error)) {
            const actions = [toAction({ id: 'workbench.action.files.saveEditors', label: ( localize(8034, "Retry")), run: () => instantiationService.invokeFunction(accessor => doSaveEditors(accessor, editors, options)) })];
            ...
            if (editorsToRevert.length > 0) {
                actions.push(toAction({ id: 'workbench.action.files.revertEditors', label: editorsToRevert.length > 1 ? ( localize(8035, "Revert All")) : ( localize(8036, "Revert")), run: () => editorService.revert(editorsToRevert) }));
            }
