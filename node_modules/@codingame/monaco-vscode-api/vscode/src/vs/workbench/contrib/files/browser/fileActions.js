...
const NEW_FILE_LABEL = ( localize2(7940, "New File..."));
const NEW_FOLDER_COMMAND_ID = 'explorer.newFolder';
const NEW_FOLDER_LABEL = ( localize2(7941, "New Folder..."));
const TRIGGER_RENAME_LABEL = ( localize(7942, "Rename..."));
const MOVE_FILE_TO_TRASH_LABEL = ( localize(7943, "Delete"));
const COPY_FILE_LABEL = ( localize(7944, "Copy"));
const PASTE_FILE_LABEL = ( localize(7945, "Paste"));
const FileCopiedContext = ( new RawContextKey('fileCopied', false));
const DOWNLOAD_COMMAND_ID = 'explorer.download';
const DOWNLOAD_LABEL = ( localize(7946, "Download..."));
const UPLOAD_COMMAND_ID = 'explorer.upload';
const UPLOAD_LABEL = ( localize(7947, "Upload..."));
...
async function deleteFiles(explorerService, workingCopyFileService, dialogService, configurationService, filesConfigurationService, elements, useTrash, skipConfirm = false, ignoreIfNotExists = false) {
    let primaryButton;
    if (useTrash) {
        primaryButton = isWindows ? ( localize(7948, "&&Move to Recycle Bin")) : ( localize(7949, "&&Move to Trash"));
    }
    else {
        primaryButton = ( localize(7950, "&&Delete"));
    }
    ...
    if (dirtyWorkingCopies.size) {
        ...
        const response = await dialogService.confirm({
            type: 'warning',
            message,
            detail: ( localize(7955, "Your changes will be lost if you don't save them.")),
            primaryButton
        });
    ...
    if (!skipConfirm) {
        ...
        if (readonlyResources.length) {
            ...
            const response = await dialogService.confirm({
                type: 'warning',
                message,
                detail: ( localize(7959, "The read-only protection will be overridden if you continue.")),
                primaryButton: ( localize(7960, "Continue"))
            });
            if (!response.confirmed) {
                return;
            }
    ...
    let confirmation;
    const deleteDetail = ( distinctElements.some(e => e.isDirectory)) ? ( localize(7961, "This action is irreversible!")) :
        distinctElements.length > 1 ? ( localize(7962, "You can restore these files using the Undo command.")) : ( localize(7963, "You can restore this file using the Undo command."));
    if (skipConfirm || (useTrash && configurationService.getValue(CONFIRM_DELETE_SETTING_KEY) === false)) {
        ...
        detail += detail ? '\n' : '';
        if (isWindows) {
            detail += distinctElements.length > 1 ? ( localize(7964, "You can restore these files from the Recycle Bin.")) : ( localize(7965, "You can restore this file from the Recycle Bin."));
        }
        else {
            detail += distinctElements.length > 1 ? ( localize(7966, "You can restore these files from the Trash.")) : ( localize(7967, "You can restore this file from the Trash."));
        }
        confirmation = await dialogService.confirm({
            ...
            primaryButton,
            checkbox: {
                label: ( localize(7968, "Do not ask me again"))
            }
        });
    }
    else {
    ...
    try {
        ...
        const options = {
            undoLabel: distinctElements.length > 1 ? ( localize(7969, "Delete {0} files", distinctElements.length)) : ( localize(7970, "Delete {0}", distinctElements[0].name)),
            progressLabel: distinctElements.length > 1 ? ( localize(7971, "Deleting {0} files", distinctElements.length)) : ( localize(7972, "Deleting {0}", distinctElements[0].name)),
        };
        ...
        if (useTrash) {
            ...
            primaryButton = ( localize(7975, "&&Delete Permanently"));
        }
        else {
            errorMessage = toErrorMessage(error, false);
            primaryButton = ( localize(7976, "&&Retry"));
        }
...
function getMoveToTrashMessage(distinctElements) {
    ...
    return { message: ( localize(7981, "Are you sure you want to delete '{0}'?", distinctElements[0].name)), detail: '' };
}
...
class GlobalCompareResourcesAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            metadata: {
                description: ( localize2(7990, "Opens a picker to select a file to diff with the active editor."))
            }
        });
    }
    async run(accessor) {
...
class ToggleAutoSaveAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(7991, "Toggle Auto Save")),
            f1: true,
            category: Categories.File,
            metadata: { description: ( localize2(7992, "Toggle the ability to save files automatically after typing")) }
        });
    }
    run(accessor) {
...
class SaveAllInGroupAction extends BaseSaveAllAction {
    ...
    static { this.LABEL = ( localize(7993, "Save All in Group")); }
    get class() {
...
let CloseGroupAction = class CloseGroupAction extends Action {
    ...
    static { this.LABEL = ( localize(7994, "Close Group")); }
...
class FocusFilesExplorer extends Action2 {
    ...
    static { this.LABEL = ( localize2(7995, "Focus on Files Explorer")); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(7996, "Moves focus to the file explorer view container."))
            }
        });
    }
    async run(accessor) {
...
class ShowActiveFileInExplorer extends Action2 {
    ...
    static { this.LABEL = ( localize2(7997, "Reveal Active File in Explorer View")); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(7998, "Reveals and selects the active file within the explorer view."))
            }
        });
    }
    async run(accessor) {
...
class OpenActiveFileInEmptyWorkspace extends Action2 {
    ...
    static { this.LABEL = ( localize2(7999, "Open Active Editor in New Empty Workspace")); }
    constructor() {
        super({
            ...
            metadata: {
                description: ( localize2(8000, "Opens the active editor in a new window with no folders open."))
            }
        });
    }
    async run(accessor) {
        ...
        if (fileResource && fileService.hasProvider(fileResource)) {
            ...
            dialogService.error(( localize(8001, "The active editor must contain an openable resource.")));
        }
    }
}
function validateFileName(pathService, item, name, os) {
    ...
    if (!name || name.length === 0 || /^\s+$/.test(name)) {
        return {
            content: ( localize(8002, "A file or folder name must be provided.")),
            severity: Severity$1.Error
        };
    }
    if (name[0] === '/' || name[0] === '\\') {
        return {
            content: ( localize(8003, "A file or folder name cannot start with a slash.")),
            severity: Severity$1.Error
        };
    }
    ...
    if (( names.some(name => /^\s|\s$/.test(name)))) {
        return {
            content: ( localize(8006, "Leading or trailing whitespace detected in file or folder name.")),
...
class CompareNewUntitledTextFilesAction extends Action2 {
    ...
    static { this.LABEL = ( localize2(8007, "Compare New Untitled Text Files")); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(8008, "Opens a new diff editor with two untitled files."))
            }
        });
    }
    async run(accessor) {
...
class CompareWithClipboardAction extends Action2 {
    ...
    static { this.LABEL = ( localize2(8009, "Compare Active File with Clipboard")); }
    ...
    async run(accessor) {
        ...
        if (resource && (fileService.hasProvider(resource) || resource.scheme === Schemas.untitled)) {
            ...
            const editorLabel = ( localize(8011, "Clipboard ↔ {0}", name));
            await editorService.openEditor({
    ...
    const onSuccess = async (value) => {
        try {
            ...
            await explorerService.applyBulkEdit([( new ResourceFileEdit(undefined, resourceToCreate, { folder: isFolder }))], {
                undoLabel: ( localize(8013, "Create {0}", value)),
                progressLabel: ( localize(8014, "Creating {0}", value)),
                ...
                if (( stat.resource.toString()) !== ( targetResource.toString())) {
                    try {
                        await explorerService.applyBulkEdit([( new ResourceFileEdit(stat.resource, targetResource))], {
                            ...
                            undoLabel: ( localize(8015, "Rename {0} to {1}", stat.name, value)),
                            progressLabel: ( localize(8016, "Renaming {0} to {1}", stat.name, value)),
                        });
                        await refreshIfSeparator(value, explorerService);
                    }
                    catch (e) {
...
const pasteFileHandler = async (accessor, fileList) => {
    ...
    if (confirmPasteNative && toPaste.files.length >= 1) {
        ...
        const confirmation = await dialogService.confirm({
            ...
            primaryButton: ( localize(8019, "&&Paste"))
        });
        if (!confirmation.confirmed) {
            return;
        }
        ...
        if (toPaste.type === 'paths') {
            const sourceTargetPairs = coalesce(await Promise.all(( toPaste.files.map(async (fileToPaste) => {
                if (( element.resource.toString()) !== ( fileToPaste.toString()) && isEqualOrParent(element.resource, fileToPaste)) {
                    throw ( new Error(( localize(8020, "File to paste is an ancestor of the destination folder"))));
                }
            ...
            if (sourceTargetPairs.length >= 1) {
                if (pasteShouldMove) {
                    ...
                    const options = {
                        confirmBeforeUndo: configurationService.getValue().explorer.confirmUndo === UndoConfirmLevel.Verbose,
                        progressLabel: sourceTargetPairs.length > 1 ? ( localize(8021, "Moving {0} files", sourceTargetPairs.length))
                            : ( localize(8022, "Moving {0}", basenameOrAuthority(sourceTargetPairs[0].target))),
                        undoLabel: sourceTargetPairs.length > 1 ? ( localize(8023, "Move {0} files", sourceTargetPairs.length))
                            : ( localize(8024, "Move {0}", basenameOrAuthority(sourceTargetPairs[0].target)))
                    };
                    await explorerService.applyBulkEdit(resourceFileEdits, options);
                }
                else {
    ...
    async function applyCopyResourceEdit(targets, resourceFileEdits) {
        ...
        const options = {
            ...
            progressLabel: targets.length > 1 ? ( localize(8026, "Copying {0} files", targets.length))
                : ( localize(8027, "Copying {0}", basenameOrAuthority(targets[0]))),
            undoLabel: targets.length > 1 ? ( localize(8028, "Paste {0} files", targets.length))
                : ( localize(8029, "Paste {0}", basenameOrAuthority(targets[0])))
        };
        await explorerService.applyBulkEdit(resourceFileEdits, options);
    }
};
...
class SetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = ( localize2(8030, "Set Active Editor Read-only in Session")); }
    constructor() {
...
class SetActiveEditorWriteableInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = ( localize2(8031, "Set Active Editor Writeable in Session")); }
    constructor() {
...
class ToggleActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = ( localize2(8032, "Toggle Active Editor Read-only in Session")); }
    constructor() {
...
class ResetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = ( localize2(8033, "Reset Active Editor Read-only in Session")); }
    constructor() {
