...
let TerminalEditorInput = class TerminalEditorInput extends EditorInput {
    ...
    async confirm(terminals) {
        const { confirmed } = await this._dialogService.confirm({
            ...
            message: ( localize(11530, "Do you want to terminate running processes?")),
            primaryButton: ( localize(11531, "&&Terminate")),
            detail: terminals.length > 1 ?
                ( terminals.map(terminal => terminal.editor.getName())).join('\n') + '\n\n' + ( localize(11532, "Closing will terminate the running processes in the terminals.")) :
                ( localize(11533, "Closing will terminate the running processes in this terminal."))
        });
