...
const COMMENTS_VIEW_TITLE = ( localize2(6349, "Comments"));
...
let CommentNodeRenderer = class CommentNodeRenderer {
    ...
    getCountString(commentCount) {
        if (commentCount > 2) {
            return localize(6350, "{0} replies", commentCount - 1);
        }
        else if (commentCount === 2) {
            return localize(6351, "1 reply");
        }
        else {
            return localize(6352, "1 comment");
        }
    }
    getRenderedComment(commentBody) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            textDescription.textContent = image.alt ? ( localize(6353, "Image: {0}", image.alt)) : ( localize(6354, "Image"));
    ...
    renderElement(node, index, templateData) {
        ...
        if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
            ...
            templateData.threadMetadata.relevance.innerText = ( localize(6355, "Outdated"));
        ...
        if (node.element.range) {
            if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
                templateData.threadMetadata.range.textContent = ( localize(6356, "[Ln {0}]", node.element.range.startLineNumber));
            }
            else {
        ...
        templateData.repliesMetadata.lastReplyDetail.textContent = ( localize(6358, "Last reply from {0}", lastComment.userName));
