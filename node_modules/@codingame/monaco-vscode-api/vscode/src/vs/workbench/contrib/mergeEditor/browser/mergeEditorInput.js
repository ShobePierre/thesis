...
let MergeEditorInput = class MergeEditorInput extends AbstractTextResourceEditorInput {
    ...
    getName() {
        return localize(9088, "Merging: {0}", super.getName());
    }
    async resolve() {
        if (!this._inputModel) {
...
function alertFocusedEditor(editor) {
    switch (editor) {
        case 'input1':
            alert(( localize(9089, "Incoming, Left Input")));
            break;
        case 'input2':
            alert(( localize(9090, "Current, Right Input")));
            break;
        case 'result':
            alert(( localize(9091, "Merge Result")));
            break;
    }
}
