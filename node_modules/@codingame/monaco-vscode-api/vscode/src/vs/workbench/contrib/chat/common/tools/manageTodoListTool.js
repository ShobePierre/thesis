...
import { LocalChatSessionUri, chatSessionResourceToId } from '../chatUri.js';
...
function createManageTodoListToolData(writeOnly, includeDescription = true) {
    ...
    return {
        ...
        displayName: ( localize(6095, 'Manage and track todo items for task planning')),
        userDescription: ( localize(6096, 'Manage and track todo items for task planning')),
...
let ManageTodoListTool = class ManageTodoListTool extends Disposable {
    ...
    async prepareToolInvocation(context, _token) {
        ...
        switch (operation) {
            ...
            case 'read': {
                message = ( localize(6097, "Read todo list"));
                break;
            }
        }
        ...
        return {
            pastTenseMessage: ( new MarkdownString(message ?? ( localize(6098, "Updated todo list")))),
            toolSpecificData: {
                kind: 'todoList',
    ...
    generatePastTenseMessage(currentTodos, newTodos) {
        if (currentTodos.length === 0) {
            return newTodos.length === 1
                ? ( localize(6099, "Created 1 todo"))
                : ( localize(6100, "Created {0} todos", newTodos.length));
        }
        ...
        if (addedTodos.length > 0) {
            return addedTodos.length === 1
                ? ( localize(6103, "Added 1 todo"))
                : ( localize(6104, "Added {0} todos", addedTodos.length));
        }
        return localize(6105, "Updated todo list");
    }
