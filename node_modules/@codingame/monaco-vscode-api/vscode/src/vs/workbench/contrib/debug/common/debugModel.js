...
class ExpressionContainer {
        ...
        try {
            ...
            const vars = ( response.body.variables.filter(v => !!v).map((v) => {
                ...
                return (new Variable(
                    ...
                    '',
                    undefined,
                    localize(7080, "Invalid variable attributes"),
                    0,
                    0,
                    undefined,
                    { kind: 'virtual' },
    ...
    async evaluateExpression(expression, session, stackFrame, context, keepLazyVars = false, location) {
        if (!session || (!stackFrame && context !== 'repl')) {
            this.value = context === 'repl' ? ( localize(7081, "Please start a debug session to evaluate expressions")) : Expression.DEFAULT_VALUE;
            this.reference = 0;
            return false;
        }
        this.session = session;
...
class Expression extends ExpressionContainer {
    static { this.DEFAULT_VALUE = ( localize(7082, "not available")); }
    ...
    get stateLabel() {
        if (this.stoppedDetails) {
            return this.stoppedDetails.description ||
                (this.stoppedDetails.reason ? ( localize(7083, "Paused on {0}", this.stoppedDetails.reason)) : ( localize(7084, "Paused")));
        }
        return localize(7085, "Running");
    }
