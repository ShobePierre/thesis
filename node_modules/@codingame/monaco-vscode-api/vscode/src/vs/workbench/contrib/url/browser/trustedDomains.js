...
const manageTrustedDomainSettingsCommand = {
    ...
    description: {
        description: ( localize2(12859, 'Manage Trusted Domains')),
        args: []
    },
...
async function configureOpenerTrustedDomainsHandler(trustedDomains, domainToConfigure, resource, quickInputService, storageService, editorService, telemetryService) {
    ...
    options.push({
        type: 'item',
        label: ( localize(12860, 'Trust {0}', domainToConfigure)),
        id: 'trust',
    ...
    if (isIP) {
        if (parsedDomainToConfigure.authority.includes(':')) {
            ...
            options.push({
                type: 'item',
                label: ( localize(12861, 'Trust {0} on all ports', base)),
                toTrust: base + ':*',
                id: 'trust'
            });
        }
        ...
        options.push({
            type: 'item',
            label: ( localize(12862, 'Trust {0} and all its subdomains', domainEnd)),
    ...
    options.push({
        type: 'item',
        label: ( localize(12863, 'Trust all domains (disables link protection)')),
        toTrust: '*',
        id: 'trust'
    });
    options.push({
        type: 'item',
        label: ( localize(12864, 'Manage Trusted Domains')),
        id: 'manage'
    });
