...
import { ExtensionState, HasOutdatedExtensionsContext, AutoUpdateConfigurationKey, AutoCheckUpdatesConfigurationKey, AutoRestartConfigurationKey, VIEWLET_ID, ExtensionRuntimeActionType } from '../common/extensions.js';
...
let Extension = class Extension {
    ...
    get resourceExtensionIconUrl() {
        if (this.resourceExtension?.manifest.icon) {
            return ( FileAccess.uriToBrowserUri(joinPath(this.resourceExtension.location, this.resourceExtension.manifest.icon)).toString(true));
        }
        return undefined;
    }
    get galleryIconUrl() {
    ...
    get outdatedTargetPlatform() {
        return !!this.local && !!this.gallery
            ...
            && semverExports.eq(this.latestVersion, this.version);
    }
    get runtimeState() {
        ...
        if (this.gallery) {
            ...
            if (!cache) {
                if (this.gallery.assets.manifest) {
                    ...
                    this.logService.error(( localize(7757, "Manifest is not found")), this.identifier.id);
                }
            }
            return cache;
        }
...
let ExtensionsWorkbenchService = class ExtensionsWorkbenchService extends Disposable {
    ...
    async updateAutoUpdateForAllExtensions(isAutoUpdateEnabled) {
        ...
        const result = await this.dialogService.confirm({
            title: ( localize(7758, "Auto Update Extensions")),
            message: isAutoUpdateEnabled
                ? ( localize(7759, "Do you want to enable auto update for all extensions?"))
                : ( localize(7760, "Do you want to disable auto update for all extensions?")),
            detail: ( localize(
                7761,
    ...
    updateExtensionsPinnedState(pinned) {
        return this.progressService.withProgress({
            ...
            title: ( localize(7762, "Updating Extensions Auto Update State")),
    ...
    async getResourceExtensions(locations, isWorkspaceScoped) {
        ...
        return ( resourceExtensions.map(
            resourceExtension => this.getInstalledExtensionMatchingLocation(resourceExtension.location)
    ...
    computeExtensionsNotifications() {
        ...
        if (invalidExtensions.length) {
            if (( invalidExtensions.some(e => e.local && e.local.manifest.engines?.vscode &&
                ...
                computedNotificiations.push({
                    message: ( localize(7766, "Invalid extensions detected. Review them.")),
                    severity: Severity$1.Warning,
                    extensions: invalidExtensions,
    ...
    async updateRunningExtensions(message = ( localize(7768, "Changing extension enablement")), auto = false) {
        const toAdd = [];
        const toRemove = [];
        ...
        for (const extension of this.extensionService.extensions) {
            ...
            if (( extensionsToCheck.some(
                e => areSameExtensions({ id: extension.identifier.value, uuid: extension.uuid }, e.local?.identifier ?? e.identifier)
            ))) {
                continue;
            }
            ...
            if (await this.extensionService.stopExtensionHosts(message, auto)) {
                ...
                if (auto) {
                    this.notificationService.notify({
                        ...
                        message: ( localize(7769, "Extensions were auto restarted to enable updates.")),
    ...
    getRuntimeState(extension) {
        ...
        const reloadActionLabel = reloadAction === ExtensionRuntimeActionType.ReloadWindow ? ( localize(7770, "reload window")) : ( localize(7771, "restart extensions"));
        if (isUninstalled) {
                ...
                if (isEnabled) {
                    ...
                    if (isSameExtensionRunning) {
                        if (!runningExtension.isUnderDevelopment && (extension.version !== runningExtension.version || extension.local.targetPlatform !== runningExtension.targetPlatform)) {
                            ...
                            return { action: reloadAction, reason: ( localize(7776, "Please {0} to enable the updated extension.", reloadActionLabel)) };
                        }
                        if (this.extensionsServers.length > 1) {
                            ...
                            if (extensionInOtherServer) {
                                if (runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.localExtensionManagementServer) {
                                    return { action: reloadAction, reason: ( localize(7777, "Please {0} to enable this extension locally.", reloadActionLabel)) };
                                }
                        ...
                        if (extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                            if (this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest)) {
                                return { action: reloadAction, reason: ( localize(7779, "Please {0} to enable this extension.", reloadActionLabel)) };
                            }
                        }
                    ...
                    if (isSameExtensionRunning) {
                        return { action: reloadAction, reason: ( localize(7780, "Please {0} to disable this extension.", reloadActionLabel)) };
                    }
                }
                return undefined;
            }
    ...
    async downloadVSIX(extensionId, versionKind) {
        ...
        if (!galleryExtension) {
            throw ( new Error(( localize(7781, "Extension '{0}' not found.", extensionId))));
        }
        ...
        for (const targetPlatform of version?.targetPlatforms ?? galleryExtension.allTargetPlatforms) {
            if (targetPlatform !== TargetPlatform.UNKNOWN && targetPlatform !== TargetPlatform.UNIVERSAL) {
                options.push({
                    label: targetPlatform === TargetPlatform.UNDEFINED ? ( localize(7782, "All Platforms")) : TargetPlatformToString(targetPlatform),
                    id: targetPlatform
                });
            }
        }
        if (options.length > 1) {
            const message = ( localize(7783, "Please select the platform for which you want to download the VSIX"));
        ...
        const result = await this.fileDialogService.showOpenDialog({
            title: ( localize(7784, "Select folder to download the VSIX")),
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
            openLabel: ( localize(7785, "Download")),
        });
        if (!result?.[0]) {
            return;
        }
        this.progressService.withProgress({ location: ProgressLocation.Notification }, async (progress) => {
            try {
                progress.report({ message: ( localize(7786, "Downloading VSIX...")) });
                ...
                this.notificationService.info(( localize(7787, "Successfully downloaded the VSIX")));
            }
            catch (error) {
                this.notificationService.error(( localize(7788, "Error while downloading the VSIX: {0}", getErrorMessage(error))));
            }
        });
    }
    async pickVersionToDownload(extensionId) {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(( localize(7789, "This extension has no other versions.")));
            return;
        }
        const picks = ( allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(( new Date(Date.parse(v.date))), true)}${v.isPreReleaseVersion ? ` (${( localize(7790, "pre-release"))})` : ''}`,
                ariaLabel: `${v.isPreReleaseVersion ? 'Pre-Release version' : 'Release version'} ${v.version}`,
                data: v,
            };
        }));
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(7791, "Select Version to Download")),
            matchOnDetail: true
        });
        return pick?.data;
    }
    ...
    async autoUpdateBuiltinExtensions() {
        ...
        await Promises.settled(( toUpdate.map(
            e => this.install(e, e.local?.preRelease ? { installPreReleaseVersion: true } : undefined)
        )));
    }
    ...
    async autoUpdateExtensions() {
        ...
        await Promises.settled(( toUpdate.map(
            e => this.install(e, e.local?.preRelease ? { installPreReleaseVersion: true, productVersion } : { productVersion })
        )));
    }
    getProductVersion() {
    ...
    async canInstall(extension) {
        if (!(extension instanceof Extension)) {
            return ( new MarkdownString()).appendText(( localize(7794, "The provided object is not an extension.")));
        }
        if (extension.isMalicious) {
            return ( new MarkdownString()).appendText(( localize(7795, "This extension is reported to be problematic.")));
        }
        if (extension.deprecationInfo?.disallowInstall) {
            return ( new MarkdownString()).appendText(( localize(7796, "This extension is disallowed to be installed.")));
        }
        if (extension.gallery) {
            if (!extension.gallery.isSigned && shouldRequireRepositorySignatureFor(extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
                return ( new MarkdownString()).appendText(( localize(7797, "This extension is not signed.")));
            }
    ...
    async install(arg, installOptions = {}, progressLocation) {
        ...
        if (arg instanceof URI) {
            ...
            if (extension?.isMalicious) {
                throw ( new Error(( localize(7795, "This extension is reported to be problematic."))));
            }
            if (gallery) {
            ...
            if (installOptions.justification) {
                ...
                buttons.push({
                    label: isString(installOptions.justification) || !installOptions.justification.action
                        ? ( localize(7802, "&&Install Extension"))
                        : ( localize(7803, "&&Install Extension and {0}", installOptions.justification.action)), run: () => true
                });
                if (!extension) {
                    buttons.push({ label: ( localize(7804, "Open Extension")), run: () => { this.open(extension); return false; } });
                }
                const result = await this.dialogService.prompt({
                    title: ( localize(7805, "Install Extension")),
                    message: extension ? ( localize(
                        7806,
                        "Would you like to install '{0}' extension from '{1}'?",
                        extension.displayName,
                        extension.publisherDisplayName
                    )) : ( localize(7807, "Would you like to install the extension?")),
                    detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                    cancelButton: true,
                    buttons,
                    checkbox: syncCheck ? {
                        label: ( localize(7808, "Sync this extension")),
                        checked: true,
                    } : undefined,
                });
                if (!result.result) {
        ...
        if (!extension) {
            throw ( new Error(( localize(7809, "Unable to install extension"))));
        }
        if (installOptions.enable) {
            if (extension.enablementState === EnablementState.DisabledWorkspace || extension.enablementState === EnablementState.DisabledGlobally) {
                if (installOptions.justification) {
                    const result = await this.dialogService.confirm({
                        title: ( localize(7810, "Enable Extension")),
                        message: ( localize(7811, "Would you like to enable '{0}' extension?", extension.displayName)),
                        detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                        primaryButton: isString(installOptions.justification) ? ( localize(7812, "&&Enable Extension")) : ( localize(7813, "&&Enable Extension and {0}", installOptions.justification.action)),
                    });
                    if (!result.confirmed) {
                        throw ( new CancellationError());
                    }
    ...
    async uninstall(e) {
        ...
        if (extension.local.isApplicationScoped && this.userDataProfilesService.profiles.length > 1) {
            const { confirmed } = await this.dialogService.confirm({
                title: ( localize(7815, "Uninstall Extension")),
                ...
                primaryButton: ( localize(7817, "Uninstall (All Profiles)"))
            });
            if (!confirmed) {
                throw ( new CancellationError());
            }
        ...
        for (const { extension } of extensionsToUninstall) {
            ...
            for (const [local, profileLocation] of installedExtensions) {
                ...
                if (( dependents.some(
                    d => d.manifest.extensionPack?.some(id => areSameExtensions({ id }, local.identifier))
                ))) {
                    continue;
                }
        ...
        return this.withProgress({
            location: ProgressLocation.Extensions,
            title: ( localize(7820, 'Uninstalling extension...')),
    ...
    doInstall(extension, installTask, progressLocation) {
        const title = extension ? ( localize(7824, "Installing '{0}' extension...", extension.displayName)) : ( localize(7825, 'Installing extension...'));
        return this.withProgress({
