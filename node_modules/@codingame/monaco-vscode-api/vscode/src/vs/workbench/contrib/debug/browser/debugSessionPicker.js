...
async function showDebugSessionMenu(accessor, selectAndStartID) {
    ...
    quickPick.placeholder = ( localize(6873, 'Search debug sessions by name'));
...
function _getPicksAndActiveItem(filter, selectAndStartID, debugService, viewsService, commandService) {
    ...
    const createDebugSessionLabel = ( localize(6874, 'Start a New Debug Session'));
...
function _getSessionInfo(session) {
    ...
    let ariaLabel = '';
    if (parentName) {
        ariaLabel = ( localize(6875, 'Session {0} spawned from {1}', label, parentName));
