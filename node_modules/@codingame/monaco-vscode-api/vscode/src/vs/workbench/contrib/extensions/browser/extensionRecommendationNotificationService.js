...
let ExtensionRecommendationNotificationService = class ExtensionRecommendationNotificationService extends Disposable {
    ...
    async promptImportantExtensionsInstallNotification(extensionRecommendations) {
        ...
        return this.promptRecommendationsNotification({ ...extensionRecommendations, extensions }, {
            ...
            onDidNeverShowRecommendedExtensionsAgain: (extensions) => {
                ...
                this.notificationService.prompt(Severity$1.Info, ( localize(7313, "Do you want to ignore all extension recommendations?")), [{
                        label: ( localize(7314, "Yes, Ignore All")),
                        run: () => this.setIgnoreRecommendationsConfig(true)
                    }, {
                        label: ( localize(7315, "No")),
    ...
    async promptWorkspaceRecommendations(recommendations) {
        ...
        await this.promptRecommendationsNotification({ extensions: recommendations, source: RecommendationSource.WORKSPACE, name: ( localize(7316, "this repository")) }, {
    ...
    async promptRecommendationsNotification({ extensions: extensionIds, source, name, searchValue }, recommendationsNotificationActions) {
        ...
        if (extensions.length === 1) {
            ...
            if (publishers.length > 2) {
                extensionsMessage = ( localize(7318, "extensions from {0}, {1} and others", publishers[0], publishers[1]));
            }
            else if (publishers.length === 2) {
                extensionsMessage = ( localize(7319, "extensions from {0} and {1}", publishers[0], publishers[1]));
            }
            else {
                extensionsMessage = ( localize(7320, "extensions from {0}", publishers[0]));
            }
        }
        ...
        const donotShowAgainLabel = source === RecommendationSource.WORKSPACE ? ( localize(7323, "Don't Show Again for this Repository"))
            : extensions.length > 1 ? ( localize(7324, "Don't Show Again for these Extensions")) : ( localize(7325, "Don't Show Again for this Extension"));
        ...
        return createCancelablePromise(async (token) => {
            ...
            choices.push({
                ...
                menu: this.userDataSyncEnablementService.isEnabled() && this.userDataSyncEnablementService.isResourceEnabled(SyncResource.Extensions) ? [{
                        label: ( localize(7327, "Install (Do not sync)")),
                        run: () => installExtensions(true)
                    }] : undefined,
            });
            choices.push(...[{
                    label: ( localize(7328, "Show Recommendations")),
                    run: async () => {
