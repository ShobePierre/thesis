...
import { isInShadowDOM, h, $, reset } from '../../../../../base/browser/dom.js';
    ...
    constructor(viewModel, modifiedBaseRange) {
        ...
        this.resultItems = derived(this, reader => {
            ...
            if (state.kind === ModifiedBaseRangeStateKind.unrecognized) {
                result.push({
                    text: ( localize(9136, "Manual Resolution")),
                    tooltip: ( localize(9137, "This conflict has been resolved manually.")),
                });
            }
            else if (state.kind === ModifiedBaseRangeStateKind.base) {
                result.push({
                    text: ( localize(9138, 'No Changes Accepted')),
                    tooltip: ( localize(
                        9139,
            ...
            const stateToggles = [];
            if (state.includesInput1) {
                stateToggles.push(command(( localize(9140, 'Remove {0}', model.input1.title)), async () => {
                    transaction((tx) => {
                ...
                }, ( localize(9141, 'Remove {0} from the result document.', model.input1.title))));
            }
            if (state.includesInput2) {
                stateToggles.push(command(( localize(9140, 'Remove {0}', model.input2.title)), async () => {
                    transaction((tx) => {
                ...
                }, ( localize(9141, 'Remove {0} from the result document.', model.input2.title))));
            }
            ...
            if (state.kind === ModifiedBaseRangeStateKind.unrecognized) {
                result.push(command(( localize(9142, 'Reset to base')), async () => {
                    transaction((tx) => {
        ...
        return derived(reader => {
            ...
            if (state.kind !== ModifiedBaseRangeStateKind.unrecognized && !state.isInputIncluded(inputNumber)) {
                if (!state.isInputIncluded(otherInputNumber) || !this.viewModel.shouldUseAppendInsteadOfAccept.read(reader)) {
                    result.push(command(( localize(9144, "Accept {0}", inputData.title)), async () => {
                        transaction((tx) => {
                    ...
                    }, ( localize(9145, "Accept {0} in the result document.", inputData.title))));
                    if (modifiedBaseRange.canBeCombined) {
                        const commandName = modifiedBaseRange.isOrderRelevant
                            ? ( localize(9146, "Accept Combination ({0} First)", inputData.title))
                            : ( localize(9147, "Accept Combination"));
                    ...
                    result.push(command(( localize(9149, "Append {0}", inputData.title)), async () => {
                        transaction((tx) => {
                            model.setState(modifiedBaseRange, state.withInputValue(inputNumber, true, false), inputNumber, tx);
                            model.telemetry.reportAcceptInvoked(inputNumber, state.includesInput(otherInputNumber));
                        });
                    }, ( localize(9150, "Append {0} to the result document.", inputData.title))));
                    if (modifiedBaseRange.canBeCombined) {
                        result.push(command(( localize(9151, "Accept Combination", inputData.title)), async () => {
                            transaction((tx) => {
                ...
                if (!model.isInputHandled(modifiedBaseRange, inputNumber).read(reader)) {
                    result.push(command(( localize(9152, 'Ignore')), async () => {
                        transaction((tx) => {
                            model.setInputHandled(modifiedBaseRange, inputNumber, true, tx);
                        });
                    }, ( localize(9153, "Don't take this side of the conflict."))));
                }
            }
            return result;
        });
