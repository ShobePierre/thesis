...
import { addDisposableListener, addStandardDisposableListener, EventType, getActiveElement, isActiveElement, isHTMLElement, hasParentWithClass, getWindow } from '../../../../../base/browser/dom.js';
...
import { FileKind, FileChangeType, FileOperationResult } from '../../../../../platform/files/common/files.js';
...
let FilesRenderer = class FilesRenderer {
    ...
    getWidgetAriaLabel() {
        return localize(8202, "Files Explorer");
    }
    get templateId() {
    ...
    renderStat(stat, label, domId, filterData, templateData) {
        ...
        if (highlightResults > 0) {
            ...
            badge.setTitleFormat(( localize(8203, "Directory contains {0} matches", highlightResults)));
    ...
    renderInputBox(container, stat, editableData) {
        ...
        const inputBox = ( new InputBox(label.element, this.contextViewService, {
            ...
            ariaLabel: ( localize(8204, "Type file name. Press Enter to confirm or Escape to cancel.")),
...
let FilesFilter = class FilesFilter {
    ...
    isVisible(stat, parentVisibility) {
        ...
        const globMatch = cached?.parsed(relative(stat.root.resource.path, stat.resource.path), stat.name, name => !!(stat.parent?.getChild(name)));
...
let FileDragAndDrop = class FileDragAndDrop {
    ...
    async handleExplorerDrop(data, target, targetIndex, targetSector, originalEvent) {
        ...
        if (confirmDragAndDrop) {
            ...
            const confirmation = await this.dialogService.confirm({
                ...
                detail,
                checkbox: {
                    label: ( localize(8209, "Do not ask me again"))
                },
                primaryButton: ( localize(8210, "&&Move"))
            });
    ...
    async doHandleExplorerDropOnCopy(sources, target) {
        ...
        await this.explorerService.applyBulkEdit(resourceFileEdits, {
            ...
            undoLabel: ( localize(8211, "Copy {0}", labelSuffix)),
            progressLabel: ( localize(8212, "Copying {0}", labelSuffix)),
        });
    ...
    async doHandleExplorerDropOnMove(sources, target) {
        ...
        const options = {
            ...
            undoLabel: ( localize(8213, "Move {0}", labelSuffix)),
            progressLabel: ( localize(8214, "Moving {0}", labelSuffix))
        };
        try {
...
function getFileOrFolderLabelSuffix(items) {
    ...
    if (items.every(i => i.isDirectory)) {
        return localize(8215, "{0} folders", items.length);
    }
    if (items.every(i => !i.isDirectory)) {
        return localize(8216, "{0} files", items.length);
    }
