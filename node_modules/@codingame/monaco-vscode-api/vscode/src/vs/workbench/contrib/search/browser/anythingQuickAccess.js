...
let AnythingQuickAccessProvider = class AnythingQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    static { this.NO_RESULTS_PICK = {
        label: ( localize(10628, "No matching results"))
    }; }
    ...
    doGetPicks(filter, options, disposables, token) {
        ...
        if (this.pickState.isQuickNavigating) {
            if (picks.length > 0) {
                picks.push({ type: 'separator', label: ( localize(10629, "recently opened")) });
            }
        ...
        return {
            ...
            additionalPicks: (async () => {
                ...
                return additionalPicks.length > 0 ? [
                    { type: 'separator', label: this.configuration.includeSymbols ? ( localize(10630, "file and symbol results")) : ( localize(10631, "file results")) },
                    ...additionalPicks
                ] : [];
            })(),
    ...
    getHelpPicks(query, token, runOptions) {
        ...
        const providers = this.lazyRegistry.value.getQuickAccessProviders()
            ...
            return {
                ...
                ariaLabel: ( localize(10632, "{0}, {1}", label, helpEntry.description)),
                accept: () => {
        ...
        if (this.quickChatService.enabled) {
            providers.push({
                label: ( localize(10633, "Open Quick Chat")),
    ...
    createAnythingPick(resourceOrEditor, configuration) {
        ...
        return {
            resource,
            label,
            ariaLabel: isDirty ? ( localize(10637, "{0} unsaved changes", labelAndDescription)) : labelAndDescription,
            description,
