...
const CONTEXT_DEBUG_TYPE = ( new RawContextKey('debugType', undefined, { type: 'string', description: ( localize(7009, "Debug type of the active debug session. For example 'python'.")) }));
...
const CONTEXT_IN_DEBUG_MODE = ( new RawContextKey('inDebugMode', false, { type: 'boolean', description: ( localize(7014, "True when debugging, false otherwise.")) }));
const CONTEXT_IN_DEBUG_REPL = ( new RawContextKey('inDebugRepl', false, { type: 'boolean', description: ( localize(7015, "True when focus is in the debug console, false otherwise.")) }));
...
const CONTEXT_BREAKPOINTS_FOCUSED = ( new RawContextKey('breakpointsFocused', true, { type: 'boolean', description: ( localize(7018, "True when the BREAKPOINTS view is focused, false otherwise.")) }));
const CONTEXT_WATCH_EXPRESSIONS_FOCUSED = ( new RawContextKey(
    'watchExpressionsFocused',
    true,
    { type: 'boolean', description: ( localize(7019, "True when the WATCH view is focused, false otherwise.")) }
));
const CONTEXT_WATCH_EXPRESSIONS_EXIST = ( new RawContextKey(
    'watchExpressionsExist',
    false,
    { type: 'boolean', description: ( localize(7020, "True when at least one watch expression exists, false otherwise.")) }
));
const CONTEXT_VARIABLES_FOCUSED = ( new RawContextKey('variablesFocused', true, { type: 'boolean', description: ( localize(7021, "True when the VARIABLES views is focused, false otherwise")) }));
...
const CONTEXT_BREAKPOINT_INPUT_FOCUSED = ( new RawContextKey(
    ...
    false,
    { type: 'boolean', description: ( localize(7023, "True when the input box has focus in the BREAKPOINTS view.")) }
));
...
const CONTEXT_CALLSTACK_FOCUSED = ( new RawContextKey('callStackFocused', true, { type: 'boolean', description: ( localize(7028, "True when the CALLSTACK view is focused, false otherwise.")) }));
...
const CONTEXT_BREAKPOINT_ITEM_IS_DATA_BYTES = ( new RawContextKey(
    'breakpointItemBytes',
    undefined,
    { type: 'boolean', description: ( localize(7032, "Whether the breakpoint item is a data breakpoint on a byte range.")) }
));
const CONTEXT_BREAKPOINT_HAS_MODES = ( new RawContextKey(
    'breakpointHasModes',
    false,
    { type: 'boolean', description: ( localize(7033, "Whether the breakpoint has multiple modes it can switch to.")) }
));
const CONTEXT_BREAKPOINT_SUPPORTS_CONDITION = ( new RawContextKey(
    'breakpointSupportsCondition',
    false,
    { type: 'boolean', description: ( localize(7034, "True when the focused breakpoint supports conditions.")) }
));
const CONTEXT_LOADED_SCRIPTS_SUPPORTED = ( new RawContextKey(
    'loadedScriptsSupported',
    false,
    { type: 'boolean', description: ( localize(7035, "True when the focused sessions supports the LOADED SCRIPTS view")) }
));
...
const CONTEXT_FOCUSED_SESSION_IS_ATTACH = ( new RawContextKey(
    ...
    false,
    { type: 'boolean', description: ( localize(7037, "True when the focused session is 'attach'.")) }
));
const CONTEXT_FOCUSED_SESSION_IS_NO_DEBUG = ( new RawContextKey(
    'focusedSessionIsNoDebug',
    false,
    { type: 'boolean', description: ( localize(7038, "True when the focused session is run without debugging.")) }
));
const CONTEXT_STEP_BACK_SUPPORTED = ( new RawContextKey('stepBackSupported', false, { type: 'boolean', description: ( localize(7039, "True when the focused session supports 'stepBack' requests.")) }));
const CONTEXT_RESTART_FRAME_SUPPORTED = ( new RawContextKey(
    'restartFrameSupported',
    false,
    { type: 'boolean', description: ( localize(7040, "True when the focused session supports 'restartFrame' requests.")) }
));
const CONTEXT_STACK_FRAME_SUPPORTS_RESTART = ( new RawContextKey(
    'stackFrameSupportsRestart',
    false,
    { type: 'boolean', description: ( localize(7041, "True when the focused stack frame supports 'restartFrame'.")) }
));
const CONTEXT_JUMP_TO_CURSOR_SUPPORTED = ( new RawContextKey(
    'jumpToCursorSupported',
    false,
    { type: 'boolean', description: ( localize(7042, "True when the focused session supports 'jumpToCursor' request.")) }
));
const CONTEXT_STEP_INTO_TARGETS_SUPPORTED = ( new RawContextKey(
    'stepIntoTargetsSupported',
    false,
    { type: 'boolean', description: ( localize(7043, "True when the focused session supports 'stepIntoTargets' request.")) }
));
const CONTEXT_BREAKPOINTS_EXIST = ( new RawContextKey('breakpointsExist', false, { type: 'boolean', description: ( localize(7044, "True when at least one breakpoint exists.")) }));
const CONTEXT_DEBUGGERS_AVAILABLE = ( new RawContextKey(
    'debuggersAvailable',
    false,
    { type: 'boolean', description: ( localize(7045, "True when there is at least one debug extensions active.")) }
));
...
const CONTEXT_SET_VARIABLE_SUPPORTED = ( new RawContextKey(
    ...
    false,
    { type: 'boolean', description: ( localize(7048, "True when the focused session supports 'setVariable' request.")) }
));
...
const CONTEXT_SET_EXPRESSION_SUPPORTED = ( new RawContextKey(
    ...
    false,
    { type: 'boolean', description: ( localize(7050, "True when the focused session supports 'setExpression' request.")) }
));
...
const CONTEXT_VARIABLE_EVALUATE_NAME_PRESENT = ( new RawContextKey(
    ...
    false,
    { type: 'boolean', description: ( localize(7057, "True when the focused variable has an 'evalauteName' field set.")) }
));
const CONTEXT_VARIABLE_IS_READONLY = ( new RawContextKey(
    'variableIsReadonly',
    false,
    { type: 'boolean', description: ( localize(7058, "True when the focused variable is read-only.")) }
));
const CONTEXT_VARIABLE_VALUE = ( new RawContextKey('variableValue', false, { type: 'string', description: ( localize(7059, "Value of the variable, present for debug visualization clauses.")) }));
const CONTEXT_VARIABLE_TYPE = ( new RawContextKey('variableType', false, { type: 'string', description: ( localize(7060, "Type of the variable, present for debug visualization clauses.")) }));
...
const CONTEXT_VARIABLE_NAME = ( new RawContextKey('variableName', false, { type: 'string', description: ( localize(7062, "Name of the variable, present for debug visualization clauses.")) }));
...
const CONTEXT_EXCEPTION_WIDGET_VISIBLE = ( new RawContextKey(
    ...
    false,
    { type: 'boolean', description: ( localize(7065, "True when the exception widget is visible.")) }
));
const CONTEXT_MULTI_SESSION_REPL = ( new RawContextKey('multiSessionRepl', false, { type: 'boolean', description: ( localize(7066, "True when there is more than 1 debug console.")) }));
const CONTEXT_MULTI_SESSION_DEBUG = ( new RawContextKey('multiSessionDebug', false, { type: 'boolean', description: ( localize(7067, "True when there is more than 1 active debug session.")) }));
const CONTEXT_DISASSEMBLE_REQUEST_SUPPORTED = ( new RawContextKey(
    'disassembleRequestSupported',
    false,
    { type: 'boolean', description: ( localize(7068, "True when the focused sessions supports disassemble request.")) }
));
const CONTEXT_DISASSEMBLY_VIEW_FOCUS = ( new RawContextKey(
    'disassemblyViewFocus',
    false,
    { type: 'boolean', description: ( localize(7069, "True when the Disassembly View is focused.")) }
));
...
const INTERNAL_CONSOLE_OPTIONS_SCHEMA = {
    ...
    description: ( localize(7073, "Controls when the internal Debug Console should open."))
};
var State;
(function (State) {
