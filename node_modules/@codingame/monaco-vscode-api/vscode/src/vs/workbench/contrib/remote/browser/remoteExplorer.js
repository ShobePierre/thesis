...
let ForwardedPortsView = class ForwardedPortsView extends Disposable {
        ...
        if (this.remoteExplorerService.tunnelModel.forwarded.size > 0) {
            this.activityBadge.value = this.activityService.showViewActivity(TUNNEL_VIEW_ID, {
                badge: ( new NumberBadge(
                    ...
                    n => n === 1 ? ( localize(10225, "1 forwarded port")) : ( localize(10226, "{0} forwarded ports", n))
                ))
            });
        }
        else {
    ...
    get entry() {
        ...
        const text = `${count}`;
        if (count === 0) {
            tooltip = ( localize(10227, "No Ports Forwarded"));
        }
        else {
        ...
        return {
            name: ( localize(10229, "Forwarded Ports")),
                        ...
                        actions: {
                            primary: [
                                ...
                                toAction({
                                    ...
                                    label: ( localize(10232, "Show Setting")),
                                    run: async () => {
...
class OnAutoForwardedAction extends Disposable {
    ...
    linkMessage() {
        return localize(10234, "[See all forwarded ports]({0})", `command:${TunnelPanel.ID}.focus`);
    }
    ...
    makePublicChoice(tunnel) {
        return {
            label: ( localize(10236, "Make Public")),
            run: async () => {
    ...
    elevateChoice(tunnel) {
        return {
            label: ( localize(10237, "Use Port {0} as Sudo...", tunnel.tunnelRemotePort)),
            run: async () => {
                await this.remoteExplorerService.close({ host: tunnel.tunnelRemoteHost, port: tunnel.tunnelRemotePort }, TunnelCloseReason.Other);
