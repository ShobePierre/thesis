...
import { isCodeEditor, getCodeEditor, isDiffEditor } from '../../../../editor/browser/editorBrowser.js';
...
const SEARCH_CANCELLED_MESSAGE = ( localize(10819, "Search was canceled before any results could be found - "));
...
let SearchView = class SearchView extends ViewPane {
    ...
    renderBody(parent) {
        ...
        const toggleQueryDetailsLabel = ( localize(10820, "Toggle Search Details"));
        ...
        const filesToIncludeTitle = ( localize(10821, "files to include"));
        ...
        this.inputPatternIncludes = this._register(this.instantiationService.createInstance(IncludePatternInputWidget, folderIncludesList, this.contextViewService, {
            ...
            placeholder: ( localize(10822, "e.g. *.ts, src/**/include")),
    ...
    replaceAll() {
        ...
        const confirmation = {
            title: ( localize(10825, "Replace All")),
            message: this.buildReplaceAllConfirmationMessage(occurrences, fileCount, replaceValue),
            primaryButton: ( localize(10826, "&&Replace"))
        };
    ...
    buildAfterReplaceAllMessage(occurrences, fileCount, replaceValue) {
        if (occurrences === 1) {
            if (fileCount === 1) {
                ...
                return localize(10828, "Replaced {0} occurrence across {1} file.", occurrences, fileCount);
            }
            if (replaceValue) {
                return localize(
                    10829,
            ...
            return localize(10830, "Replaced {0} occurrence across {1} files.", occurrences, fileCount);
        }
        if (fileCount === 1) {
            if (replaceValue) {
                return localize(
            ...
            return localize(10832, "Replaced {0} occurrences across {1} file.", occurrences, fileCount);
        }
        if (replaceValue) {
            return localize(
                10833,
    ...
    buildReplaceAllConfirmationMessage(occurrences, fileCount, replaceValue) {
        ...
        if (occurrences === 1) {
            if (fileCount === 1) {
                ...
                return localize(10836, "Replace {0} occurrence across {1} file?", occurrences, fileCount);
            }
            if (displayReplaceValue) {
                return localize(
                    10837,
            ...
            return localize(10838, "Replace {0} occurrence across {1} files?", occurrences, fileCount);
        }
        if (fileCount === 1) {
            if (displayReplaceValue) {
                return localize(
            ...
            return localize(10840, "Replace {0} occurrences across {1} file?", occurrences, fileCount);
        }
        if (displayReplaceValue) {
            return localize(
                10841,
        ...
        return localize(10842, "Replace {0} occurrences across {1} files?", occurrences, fileCount);
    }
    clearMessage() {
    ...
    clearSearchResults(clearInput = true) {
        ...
        this.tree.ariaLabel = ( localize(10843, "Empty Search"));
    ...
    validateQuery(query) {
        ...
        return Promise.all(folderQueriesExistP).then(existResults => {
            ...
            if (!query.folderQueries.length || existingFolderQueries.length) {
                ...
                const searchPathNotFoundError = ( localize(10844, "Search path not found: {0}", nonExistantPath));
    ...
    appendSearchWithAIButton(messageEl) {
        const searchWithAIButtonTooltip = appendKeyBindingLabel(( localize(10845, "Search with AI.")), this.keybindingService.lookupKeybinding(SearchCommandIds.SearchWithAIActionId));
        const searchWithAIButtonText = ( localize(10846, "Search with AI"));
        ...
        if (!allResults) {
            ...
            if (!completed) {
                ...
                if (hasIncludes && hasExcludes) {
                    ...
                    message = ( localize(10852, "No results found in '{0}' - ", includePatternText));
                }
                else if (hasExcludes) {
                    message = ( localize(10853, "No results found excluding '{0}' - ", excludePatternText));
                }
                else {
                    message = ( localize(
                        10854,
            ...
            if (!completed) {
                const searchAgainButton = this.messageDisposables.add(( new SearchLinkButton(( localize(10855, "Search again")), () => this.triggerQueryChange({ preserveFocus: false }), this.hoverService)));
                append(messageEl, searchAgainButton.element);
            }
            else if (hasIncludes || hasExcludes) {
                const searchAgainButton = this.messageDisposables.add(( new SearchLinkButton(( localize(10856, "Search again in all files")), this.onSearchAgain.bind(this), this.hoverService)));
                append(messageEl, searchAgainButton.element);
            }
            else {
                const openSettingsButton = this.messageDisposables.add(( new SearchLinkButton(( localize(10857, "Open Settings")), this.onOpenSettings.bind(this), this.hoverService)));
    ...
    updateSearchResultCount(disregardExcludesAndIgnores, onlyOpenEditors, clear = false) {
        ...
        if (fileCount > 0) {
            if (disregardExcludesAndIgnores) {
                const excludesDisabledMessage = ' - ' + ( localize(10861, "exclude settings and ignore files are disabled")) + ' ';
                const enableExcludesButton = this.messageDisposables.add(( new SearchLinkButton(( localize(10862, "enable")), this.onEnableExcludes.bind(this), this.hoverService, ( localize(10863, "Use Exclude Settings and Ignore Files")))));
                append(messageEl, $('span', undefined, excludesDisabledMessage, '(', enableExcludesButton.element, ')'));
            }
            if (onlyOpenEditors) {
                const searchingInOpenMessage = ' - ' + ( localize(10864, "searching only in open files")) + ' ';
                const disableOpenEditorsButton = this.messageDisposables.add(( new SearchLinkButton(( localize(10865, "disable")), this.onDisableSearchInOpenEditors.bind(this), this.hoverService, ( localize(10866, "Search in entire workspace")))));
                append(messageEl, $('span', undefined, searchingInOpenMessage, '(', disableOpenEditorsButton.element, ')'));
            }
            append(messageEl, ' - ');
            const openInEditorTooltip = appendKeyBindingLabel(( localize(10867, "Copy current search results to an editor")), this.keybindingService.lookupKeybinding(SearchCommandIds.OpenInEditorCommandId));
            const openInEditorButton = this.messageDisposables.add(( new SearchLinkButton(( localize(10868, "Open in editor")), () => this.instantiationService.invokeFunction(createEditorFromSearchResult, this.searchResult, this.searchIncludePattern.getValue(), this.searchExcludePattern.getValue(), this.searchIncludePattern.onlySearchInOpenEditors()), this.hoverService, openInEditorTooltip)));
    ...
    updateKeywordSuggestionUI(keyword) {
        ...
        if (this._cachedKeywords.length > 0) {
            ...
            const resultMsg = ( localize(10869, "Search instead for: "));
            append(messageEl, resultMsg);
            const button = this.messageDisposables.add(( new SearchLinkButton(
                keyword.keyword,
    ...
    buildResultCountMessage(resultCount, fileCount) {
        if (resultCount === 1 && fileCount === 1) {
            return localize(10870, "{0} result in {1} file", resultCount, fileCount);
        }
        else if (resultCount === 1) {
            return localize(10871, "{0} result in {1} files", resultCount, fileCount);
        }
        else if (fileCount === 1) {
            return localize(10872, "{0} results in {1} file", resultCount, fileCount);
        }
        else {
            return localize(10873, "{0} results in {1} files", resultCount, fileCount);
        }
    }
    showSearchWithoutFolderMessage() {
        ...
        const openFolderButton = this.messageDisposables.add(( new SearchLinkButton(( localize(10875, "Open Folder")), () => {
