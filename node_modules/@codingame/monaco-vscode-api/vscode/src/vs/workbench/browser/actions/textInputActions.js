...
function createTextInputActions(clipboardService, logService) {
    return [
        toAction({ id: 'undo', label: ( localize(2896, "Undo")), run: () => getActiveDocument().execCommand('undo') }),
        toAction({ id: 'redo', label: ( localize(2897, "Redo")), run: () => getActiveDocument().execCommand('redo') }),
        ( new Separator()),
        toAction({
            id: 'editor.action.clipboardCutAction', label: ( localize(2898, "Cut")), run: () => {
                logService.trace('TextInputActionsProvider#cut');
                getActiveDocument().execCommand('cut');
            }
        }),
        toAction({
            id: 'editor.action.clipboardCopyAction', label: ( localize(2899, "Copy")), run: () => {
        ...
        ( new Separator()),
        toAction({ id: 'editor.action.selectAll', label: ( localize(2901, "Select All")), run: () => getActiveDocument().execCommand('selectAll') })
    ];
}
