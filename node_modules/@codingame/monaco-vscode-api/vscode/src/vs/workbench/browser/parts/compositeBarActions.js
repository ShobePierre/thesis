...
let CompositeBarActionViewItem = class CompositeBarActionViewItem extends BaseActionViewItem {
    ...
    computeTitle() {
        ...
        let title = this.keybindingLabel ? ( localize(3013, "{0} ({1})", this.compositeBarActionItem.name, this.keybindingLabel)) : this.compositeBarActionItem.name;
...
class CompositeOverflowActivityAction extends CompositeBarAction {
    constructor(showMenu) {
        super({
            ...
            name: ( localize(3014, "Additional Views")),
...
let CompositeOverflowActivityActionViewItem = class CompositeOverflowActivityActionViewItem extends CompositeBarActionViewItem {
    ...
    getActions() {
        return (this.getOverflowingComposites().map(composite => {
            ...
            if (suffix) {
                action.label = ( localize(3015, "{0} ({1})", composite.name, suffix));
            }
            else {
...
let CompositeActionViewItem = class CompositeActionViewItem extends CompositeBarActionViewItem {
    ...
    showContextMenu(container) {
        ...
        if (isPinned) {
            this.toggleCompositePinnedAction.label = ( localize(3016, "Hide '{0}'", this.compositeBarActionItem.name));
            this.toggleCompositePinnedAction.checked = false;
            this.toggleCompositePinnedAction.enabled = this.compositeBar.getPinnedCompositeIds().length > 1;
        }
        else {
            this.toggleCompositePinnedAction.label = ( localize(3017, "Keep '{0}'", this.compositeBarActionItem.name));
        ...
        if (isBadgeEnabled) {
            this.toggleCompositeBadgeAction.label = ( localize(3018, "Hide Badge"));
        }
        else {
            this.toggleCompositeBadgeAction.label = ( localize(3019, "Show Badge"));
        }
...
class ToggleCompositePinnedAction extends Action {
    constructor(activity, compositeBar) {
        super('show.toggleCompositePinned', activity ? activity.name : ( localize(3020, "Toggle View Pinned")));
...
class ToggleCompositeBadgeAction extends Action {
    constructor(compositeBarActionItem, compositeBar) {
        super('show.toggleCompositeBadge', compositeBarActionItem ? compositeBarActionItem.name : ( localize(3021, "Toggle View Badge")));
