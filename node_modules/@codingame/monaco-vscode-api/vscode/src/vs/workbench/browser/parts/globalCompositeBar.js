...
import { AnchorAxisAlignment, AnchorAlignment } from '../../../base/browser/ui/contextview/contextview.js';
...
let GlobalCompositeBar = class GlobalCompositeBar extends Disposable {
    ...
    static { this.ACCOUNTS_ICON = registerIcon('accounts-view-bar-icon', Codicon.account, ( localize(3486, "Accounts icon in the view bar."))); }
    constructor(contextMenuActionsProvider, colors, activityHoverOptions, configurationService, instantiationService, storageService, extensionService) {
        ...
        this.globalActivityActionBar = this._register(( new ActionBar(this.element, {
            actionViewItemProvider: (action, options) => {
                ...
                if (action.id === ACCOUNTS_ACTIVITY_ID) {
                    return this.instantiationService.createInstance(AccountsActivityActionViewItem, this.contextMenuActionsProvider, {
                        ...
                        actions.unshift(...[
                            toAction({ id: 'hideAccounts', label: ( localize(3487, "Hide Accounts")), run: () => setAccountsActionVisible(storageService, false) }),
                            ( new Separator())
                        ]);
                    });
                }
            ...
            ariaLabel: ( localize(3488, "Manage")),
    ...
    getContextMenuActions() {
        return [toAction({ id: 'toggleAccountsVisibility', label: ( localize(3489, "Accounts")), checked: this.accountsVisibilityPreference, run: () => this.accountsVisibilityPreference = !this.accountsVisibilityPreference })];
    }
...
let AccountsActivityActionViewItem = class AccountsActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
    ...
    async resolveMainMenuActions(accountsMenu, disposables) {
        ...
        if (!this.initialized) {
            const noAccountsAvailableAction = disposables.add(( new Action('noAccountsAvailable', ( localize(3490, "Loading...")), undefined, false)));
            ...
            for (const providerId of registeredProviders) {
                ...
                if (!accounts) {
                    if (( this.problematicProviders.has(providerId))) {
                        const providerUnavailableAction = disposables.add(( new Action('providerUnavailable', ( localize(3491, '{0} is currently unavailable', provider.label)), undefined, false)));
                ...
                for (const account of accounts) {
                    const manageExtensionsAction = toAction({
                        ...
                        label: ( localize(3492, "Manage Trusted Extensions")),
                        enabled: true,
                    ...
                    if (canUseMcp) {
                        const manageMCPAction = toAction({
                            id: `configureSessions${account.label}`,
                            label: ( localize(3493, "Manage Trusted MCP Servers")),
                            enabled: true,
            ...
            for (const providerId of dynamicProviders) {
                ...
                const manageDynamicAuthProvidersAction = toAction({
                    ...
                    label: ( localize(3495, "Manage Dynamic Authentication Providers...")),
                    enabled: true,
...
function simpleActivityContextMenuActions(storageService, isAccount) {
    ...
    if (isAccount) {
        currentElementContextMenuActions.push(toAction({ id: 'hideAccounts', label: ( localize(3487, "Hide Accounts")), run: () => setAccountsActionVisible(storageService, false) }), ( new Separator()));
    }
    return [
        ...currentElementContextMenuActions,
        toAction({ id: 'toggle.hideAccounts', label: ( localize(3489, "Accounts")), checked: isAccountsActionVisible(storageService), run: () => setAccountsActionVisible(storageService, !isAccountsActionVisible(storageService)) }),
        toAction({ id: 'toggle.hideManage', label: ( localize(3488, "Manage")), checked: true, enabled: false, run: () => { throw ( new Error('"Manage" can not be hidden')); } })
    ];
}
