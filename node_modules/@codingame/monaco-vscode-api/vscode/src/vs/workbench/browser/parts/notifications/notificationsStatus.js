...
let NotificationsStatus = class NotificationsStatus extends Disposable {
    ...
    updateNotificationsCenterStatusItem() {
        ...
        if (this.notificationService.getFilter() === NotificationsFilter.ERROR) {
            statusProperties = {
                ...
                ariaLabel: ( localize(3543, "Do Not Disturb")),
                tooltip: ( localize(3544, "Do Not Disturb Mode is Enabled"))
            };
        }
        if (!this.notificationsCenterStatusItem) {
            this.notificationsCenterStatusItem = this.statusbarService.addEntry(statusProperties, 'status.notifications', StatusbarAlignment.RIGHT, Number.NEGATIVE_INFINITY );
        }
        else {
    ...
    getTooltip(notificationsInProgress) {
        if (this.isNotificationsCenterVisible) {
            return localize(3545, "Hide Notifications");
        }
        if (this.model.notifications.length === 0) {
            return localize(3546, "No Notifications");
        }
        if (notificationsInProgress === 0) {
            if (this.newNotificationsCount === 0) {
                return localize(3547, "No New Notifications");
            }
            if (this.newNotificationsCount === 1) {
                return localize(3548, "1 New Notification");
            }
            return localize(3549, "{0} New Notifications", this.newNotificationsCount);
        }
        if (this.newNotificationsCount === 0) {
            return localize(3550, "No New Notifications ({0} in progress)", notificationsInProgress);
        }
        if (this.newNotificationsCount === 1) {
            return localize(3551, "1 New Notification ({0} in progress)", notificationsInProgress);
        }
        return localize(
            3552,
    ...
    doSetStatusMessage(item) {
        ...
        let showHandle = setTimeout(() => {
            statusMessageEntry = this.statusbarService.addEntry({
                name: ( localize(3553, "Status Message")),
                text: message,
                ariaLabel: message
