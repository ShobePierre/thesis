...
import { getWindows, getWindowsCount, getWindow, getActiveWindow, getWindowById, addDisposableListener, EventType, EventHelper, windowOpenWithSuccess, windowOpenPopup, windowOpenNoOpener } from '../../base/browser/dom.js';
...
let BaseWindow = class BaseWindow extends Disposable {
    ...
    static async confirmOnShutdown(accessor, reason) {
        ...
        const message = reason === ShutdownReason.QUIT ?
            (isMacintosh ? ( localize(3682, "Are you sure you want to quit?")) : ( localize(3683, "Are you sure you want to exit?"))) :
            ( localize(3684, "Are you sure you want to close the window?"));
        const primaryButton = reason === ShutdownReason.QUIT ?
            (isMacintosh ? ( localize(3685, "&&Quit")) : ( localize(3686, "&&Exit"))) :
            ( localize(3687, "&&Close Window"));
        const res = await dialogService.confirm({
            message,
            primaryButton,
            checkbox: {
                label: ( localize(3688, "Do not ask me again"))
            }
        });
    ...
    onWillShutdown() {
        Event.toPromise(Event.any(Event.once(( new DomEmitter(mainWindow.document.body, EventType.KEY_DOWN, true)).event), Event.once(( new DomEmitter(mainWindow.document.body, EventType.MOUSE_DOWN, true)).event))).then(async () => {
            ...
            await this.dialogService.prompt({
                ...
                message: ( localize(3689, "An unexpected error occurred that requires a reload of this page.")),
                detail: ( localize(3690, "The workbench was unexpectedly disposed while running.")),
                buttons: [
                    {
                        label: ( localize(3691, "&&Reload")),
                ...
                if (matchesScheme(href, Schemas.http) || matchesScheme(href, Schemas.https)) {
                    ...
                    const showProtocolUrlOpenedDialog = async () => {
                        ...
                        await this.hostService.withExpectedShutdown(() => this.dialogService.prompt({
                            ...
                            message: ( localize(3699, "All done. You can close this tab now.")),
                            detail,
                            buttons,
                            cancelButton: true
                        }));
