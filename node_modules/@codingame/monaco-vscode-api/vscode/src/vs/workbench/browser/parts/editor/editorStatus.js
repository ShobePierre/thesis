...
const nlsSingleSelectionRange = ( localize(3417, "Ln {0}, Col {1} ({2} selected)"));
const nlsSingleSelection = ( localize(3418, "Ln {0}, Col {1}"));
const nlsMultiSelectionRange = ( localize(3419, "{0} selections ({1} characters selected)"));
const nlsMultiSelection = ( localize(3420, "{0} selections"));
const nlsEOLLF = ( localize(3421, "LF"));
const nlsEOLCRLF = ( localize(3422, "CRLF"));
let EditorStatus = class EditorStatus extends Disposable {
    ...
    async showIndentationPicker() {
        ...
        if (!activeTextEditorControl) {
            return this.quickInputService.pick([{ label: ( localize(3423, "No text editor active at this time")) }]);
        }
        if (this.editorService.activeEditor?.isReadonly()) {
            return this.quickInputService.pick([{ label: ( localize(3424, "The active code editor is read-only.")) }]);
        }
        const picks = ( [
        ...
        picks.splice(3, 0, { type: 'separator', label: ( localize(3425, "convert file")) });
        picks.unshift({ type: 'separator', label: ( localize(3426, "change view")) });
        const action = await this.quickInputService.pick(picks, { placeHolder: ( localize(3427, "Select Action")), matchOnDetail: true });
        return action?.run();
    }
    updateTabFocusModeElement(visible) {
        if (visible) {
            if (!this.tabFocusModeElement.value) {
                const text = ( localize(3428, "Tab Moves Focus"));
                this.tabFocusModeElement.value = this.statusbarService.addEntry({
                    name: ( localize(3429, "Accessibility Mode")),
                    text,
                    ariaLabel: text,
                    tooltip: ( localize(3430, "Disable Accessibility Mode")),
    ...
    updateInputModeElement(inputMode) {
        if (inputMode === 'overtype') {
            if (!this.inputModeElement.value) {
                const text = ( localize(3431, 'OVR'));
                const name = ( localize(3432, "Enable Insert Mode"));
    ...
    updateColumnSelectionModeElement(visible) {
        if (visible) {
            if (!this.columnSelectionModeElement.value) {
                const text = ( localize(3433, "Column Selection"));
                this.columnSelectionModeElement.value = this.statusbarService.addEntry({
                    name: ( localize(3434, "Column Selection Mode")),
                    text,
                    ariaLabel: text,
                    tooltip: ( localize(3435, "Disable Column Selection Mode")),
    ...
    updateSelectionElement(text) {
        ...
        const props = {
            name: ( localize(3436, "Editor Selection")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3437, "Go to Line/Column")),
    ...
    updateIndentationElement(text) {
        ...
        const props = {
            name: ( localize(3438, "Editor Indentation")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3439, "Select Indentation")),
    ...
    updateEncodingElement(text) {
        ...
        const props = {
            name: ( localize(3440, "Editor Encoding")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3441, "Select Encoding")),
    ...
    updateEOLElement(text) {
        ...
        const props = {
            name: ( localize(3442, "Editor End of Line")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3443, "Select End of Line Sequence")),
    ...
    updateLanguageIdElement(text) {
        ...
        const props = {
            name: ( localize(3444, "Editor Language")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3445, "Select Language Mode")),
    ...
    updateMetadataElement(text) {
        ...
        const props = {
            name: ( localize(3446, "File Information")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3447, "File Information"))
        };
            ...
            if (model) {
                ...
                update.indentation = (modelOpts.insertSpaces
                    ? modelOpts.tabSize === modelOpts.indentSize
                        ? ( localize(3448, "Spaces: {0}", modelOpts.indentSize))
                        : ( localize(
                    3449,
                    "Spaces: {0} (Tab Size: {1})",
                    ...
                    : ( localize(3450, "Tab Size: {0}", modelOpts.tabSize)));
            }
        }
        ...
        if (this.hasToUpdateStatus(previousMarker, this.currentMarker)) {
            if (this.currentMarker) {
                ...
                if (!this.statusBarEntryAccessor.value) {
                    this.statusBarEntryAccessor.value = this.statusbarService.addEntry({ name: ( localize(3451, "Current Problem")), text, ariaLabel: text }, 'statusbar.currentProblem', StatusbarAlignment.LEFT);
                }
                else {
                    this.statusBarEntryAccessor.value.update({ name: ( localize(3451, "Current Problem")), text, ariaLabel: text });
                }
            }
            else {
...
let ShowLanguageExtensionsAction = class ShowLanguageExtensionsAction extends Action {
    ...
    constructor(fileExtension, commandService, galleryService) {
        super(ShowLanguageExtensionsAction_1.ID, ( localize(3452, "Search Marketplace Extensions for '{0}'...", fileExtension)));
...
class ChangeLanguageAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(3453, 'Change Language Mode')),
            f1: true,
            keybinding: {
            ...
            metadata: {
                description: ( localize(3454, "Change the language mode of the active text editor.")),
                args: [
                    {
                        name: ( localize(3455, "The name of the language mode to change to.")),
    ...
    async run(accessor, languageMode) {
        ...
        if (!activeTextEditorControl) {
            await quickInputService.pick([{ label: ( localize(3423, "No text editor active at this time")) }]);
            return;
        }
        ...
        const picks = ( languages
            ...
            if (currentLanguageName === languageName) {
                description = ( localize(3456, "({0}) - Configured Language", languageId));
            }
            else {
                description = ( localize(3457, "({0})", languageId));
            }
            return {
                label: languageName,
                meta: extensions,
        ...
        picks.unshift({ type: 'separator', label: ( localize(3458, "languages (identifier)")) });
        ...
        if (hasLanguageSupport && resource) {
            ...
            configureLanguageSettings = { label: ( localize(3459, "Configure '{0}' language based settings...", currentLanguageName)) };
            picks.unshift(configureLanguageSettings);
            configureLanguageAssociations = { label: ( localize(3460, "Configure File Association for '{0}'...", ext)) };
            picks.unshift(configureLanguageAssociations);
        }
        const autoDetectLanguage = { label: ( localize(3461, "Auto Detect")) };
        if (textModel && textModel.getValueLength() > 0) {
            picks.unshift(autoDetectLanguage);
        }
        const pick = typeof languageMode === 'string' ? { label: languageMode } : await quickInputService.pick(picks, { placeHolder: ( localize(3462, "Select Language Mode")), matchOnDescription: true });
        if (!pick) {
            return;
        }
    ...
    configureFileAssociation(resource, languageService, quickInputService, configurationService) {
        ...
        const picks = ( languages.map(({ languageName, languageId }) => {
            return {
                ...
                label: languageName,
                iconClasses: getIconClassesForLanguageId(languageId),
                description: (languageId === currentAssociation) ? ( localize(3463, "Current Association")) : undefined
            };
        }));
        setTimeout(async () => {
            const language = await quickInputService.pick(picks, { placeHolder: ( localize(3464, "Select Language Mode to Associate with '{0}'", extension || base)) });
            if (language) {
...
class ChangeEOLAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3465, 'Change End of Line Sequence')),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (editorService.activeEditor?.isReadonly()) {
            await quickInputService.pick([{ label: ( localize(3424, "The active code editor is read-only.")) }]);
            return;
        }
        ...
        const eol = await quickInputService.pick(EOLOptions, { placeHolder: ( localize(3466, "Select End of Line Sequence")), activeItem: EOLOptions[selectedIndex] });
        if (eol) {
...
class ChangeEncodingAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3467, 'Change File Encoding')),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (!encodingSupport) {
            await quickInputService.pick([{ label: ( localize(3468, "No file active at this time")) }]);
            return;
        }
        const saveWithEncodingPick = { label: ( localize(3469, "Save with Encoding")) };
        const reopenWithEncodingPick = { label: ( localize(3470, "Reopen with Encoding")) };
        ...
        if (encodingSupport instanceof UntitledTextEditorInput) {
            ...
            action = await quickInputService.pick([reopenWithEncodingPick, saveWithEncodingPick], { placeHolder: ( localize(3427, "Select Action")), matchOnDetail: true });
        }
        if (!action) {
            return;
        }
        ...
        if (guessedEncoding && configuredEncoding !== guessedEncoding && SUPPORTED_ENCODINGS[guessedEncoding]) {
            ...
            picks.unshift({ id: guessedEncoding, label: SUPPORTED_ENCODINGS[guessedEncoding].labelLong, description: ( localize(3471, "Guessed from content")) });
        }
        const encoding = await quickInputService.pick(picks, {
            placeHolder: isReopenWithEncoding ? ( localize(3472, "Select File Encoding to Reopen File")) : ( localize(3473, "Select File Encoding to Save with")),
        ...
        if (typeof encoding.id !== 'undefined' && activeEncodingSupport) {
            if (isReopenWithEncoding && editorService.activeEditorPane.input.isDirty()) {
                const { confirmed } = await dialogService.confirm({
                    ...
                    detail: ( localize(3475, "This will discard any unsaved changes.")),
                    primaryButton: ( localize(3476, "Discard Changes and Reopen"))
                });
                if (!confirmed) {
                    return;
                }
