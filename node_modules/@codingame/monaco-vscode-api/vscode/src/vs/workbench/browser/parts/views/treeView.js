...
const noDataProviderMessage = ( localize(3663, "There is no data provider registered that can provide view data."));
...
let AbstractTreeView = class AbstractTreeView extends Disposable {
    ...
    initializeShowCollapseAllAction(startingValue = false) {
        if (!this.collapseAllContext) {
            this.collapseAllContextKey = ( new RawContextKey(`treeView.${this.id}.enableCollapseAll`, startingValue, ( localize(3664, "Whether the tree view with id {0} enables collapse all.", this.id))));
    ...
    initializeShowRefreshAction(startingValue = false) {
        if (!this.refreshContext) {
            this.refreshContextKey = ( new RawContextKey(`treeView.${this.id}.enableRefresh`, startingValue, ( localize(3665, "Whether the tree view with id {0} enables refresh.", this.id))));
        ...
        this._register(registerAction2(class extends Action2 {
            constructor() {
                super({
                    ...
                    title: ( localize(3667, "Collapse All")),
                    menu: {
                        id: MenuId.ViewTitle,
    ...
    processMessage(message, disposables) {
        ...
        for (const line of lines) {
            ...
            if (linkedText.nodes.length === 1 && typeof linkedText.nodes[0] !== 'string') {
                ...
                const rendered = this.markdownRendererService.render(( new MarkdownString(
                    line,
    ...
    getHover(label, resource, node) {
        if (!(node instanceof ResolvableTreeItem) || !node.hasResolve) {
            if (resource && !node.tooltip) {
                ...
                return { markdown: node.tooltip, markdownNotSupportedFallback: resource ? undefined : renderAsPlaintext(node.tooltip) };
            }
