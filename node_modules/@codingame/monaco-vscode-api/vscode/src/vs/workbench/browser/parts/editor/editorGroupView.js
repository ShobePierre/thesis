...
let EditorGroupView = EditorGroupView_1 = class EditorGroupView extends Themable {
    ...
    createContainerToolbar() {
        ...
        const containerToolbar = this._register(( new ActionBar(toolbarContainer, {
            ariaLabel: ( localize(3380, "Empty editor group actions")),
    ...
    updateContainer() {
        if (this.isEmpty) {
            ...
            this.element.setAttribute('aria-label', ( localize(3381, "{0} (empty)", this.ariaLabel)));
        }
        else {
    ...
    get label() {
        if (this.groupsLabel) {
            return localize(3382, "{0}: Group {1}", this.groupsLabel, this._index + 1);
        }
        return localize(3383, "Group {0}", this._index + 1);
    }
    get ariaLabel() {
        if (this.groupsLabel) {
            return localize(3384, "{0}: Editor Group {1}", this.groupsLabel, this._index + 1);
        }
        return localize(3385, "Editor Group {0}", this._index + 1);
    }
    get disposed() {
    ...
    doMoveOrCopyEditorAcrossGroups(editor, target, openOptions, internalOptions) {
        ...
        if (!keepCopy || editor.hasCapability(EditorInputCapabilities.Singleton) ) {
            ...
            if (typeof canMoveVeto === 'string') {
                this.dialogService.error(canMoveVeto, ( localize(3386, "Try saving or reverting the editor first and then try again.")));
                return false;
            }
        }
