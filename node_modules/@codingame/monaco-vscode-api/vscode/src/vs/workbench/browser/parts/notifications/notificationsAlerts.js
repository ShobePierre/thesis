...
class NotificationsAlerts extends Disposable {
    ...
    doTriggerAriaAlert(notification) {
        ...
        if (notification.severity === Severity$1.Error) {
            alertText = ( localize(3516, "Error: {0}", (notification.message.linkedText.toString())));
        }
        else if (notification.severity === Severity$1.Warning) {
            alertText = ( localize(3517, "Warning: {0}", (notification.message.linkedText.toString())));
        }
        else {
            alertText = ( localize(3518, "Info: {0}", (notification.message.linkedText.toString())));
        }
        alert(alertText);
    }
}
