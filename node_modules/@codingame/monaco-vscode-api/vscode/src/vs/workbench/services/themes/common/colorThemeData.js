...
async function _loadColorTheme(extensionResourceLoaderService, themeLocation, result) {
    if (extname(themeLocation) === '.json') {
        ...
        if (errors.length > 0) {
            ...
            return Promise.reject(( new Error(( localize(14298, "Invalid format for JSON theme file: Object expected.")))));
        }
    ...
    return extensionResourceLoaderService.readExtensionResource(themeLocation).then(content => {
        try {
            ...
            if (!Array.isArray(settings)) {
                return Promise.reject(( new Error(( localize(14302, "Problem parsing tmTheme file: {0}. 'settings' is not array.")))));
            }
            convertSettings(settings, result);
            return Promise.resolve(null);
        }
        catch (e) {
            return Promise.reject(( new Error(( localize(14303, "Problems parsing tmTheme file: {0}", e.message)))));
        }
    }, error => {
        return Promise.reject(( new Error(( localize(
            14304,
