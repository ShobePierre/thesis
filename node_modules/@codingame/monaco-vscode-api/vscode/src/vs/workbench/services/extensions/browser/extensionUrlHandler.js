    ...
    async handleURL(uri, options) {
        ...
        if (!trusted) {
            ...
            const result = await this.dialogService.confirm({
                message: ( localize(13930, "Allow '{0}' extension to open this URI?", extensionDisplayName)),
                checkbox: {
                    label: ( localize(13931, "Do not ask me again for this extension")),
                },
                primaryButton: ( localize(13932, "&&Open")),
                custom: {
                    markdownDetails: [{
    ...
    async handleUnhandledURL(uri, extensionId, options) {
        try {
            await this.commandService.executeCommand('workbench.extensions.installExtension', extensionId, {
                justification: {
                    reason: `${( localize(13933, "This extension wants to open a URI:"))}\n${( uri.toString())}`,
                    action: ( localize(13934, "Open URI"))
                },
                enable: true,
        ...
        if (extension) {
            ...
            const result = await this.dialogService.confirm({
                ...
                primaryButton: ( localize(13936, "&&Reload Window and Open"))
            });
            if (!result.confirmed) {
                return;
            }
...
class ManageAuthorizedExtensionURIsAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(13937, 'Manage Authorized Extension URIs...')),
            category: ( localize2(13938, 'Extensions')),
            menu: {
    ...
    async run(accessor) {
        ...
        if (items.length === 0) {
            await quickInputService.pick([{ label: ( localize(13939, 'There are currently no authorized extension URIs.')) }]);
            return;
        }
