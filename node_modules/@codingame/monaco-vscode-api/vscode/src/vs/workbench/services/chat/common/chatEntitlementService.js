...
let ChatEntitlementRequests = ChatEntitlementRequests_1 = class ChatEntitlementRequests extends Disposable {
    ...
    async signUpFree(sessions) {
        ...
        if (!response) {
            const retry = await this.onUnknownSignUpError(( localize(13691, "No response received.")), '[chat entitlement] sign-up: no response');
        ...
        if (response.res.statusCode && response.res.statusCode !== 200) {
            ...
            const retry = await this.onUnknownSignUpError(( localize(13692, "Unexpected status code {0}.", response.res.statusCode)), `[chat entitlement] sign-up: unexpected status code ${response.res.statusCode}`);
        ...
        if (!responseText) {
            const retry = await this.onUnknownSignUpError(( localize(13693, "Response has no contents.")), '[chat entitlement] sign-up: response has no content');
        ...
        try {
            ...
            const retry = await this.onUnknownSignUpError(( localize(13694, "Invalid response contents.")), `[chat entitlement] sign-up: error parsing response (${err})`);
    ...
    async onUnknownSignUpError(detail, logMessage) {
        ...
        if (!this.lifecycleService.willShutdown) {
            const { confirmed } = await this.dialogService.confirm({
                ...
                detail,
                primaryButton: ( localize(13696, "Retry"))
            });
            return confirmed;
        }
        return false;
    ...
    onUnprocessableSignUpError(logMessage, logDetails) {
        ...
        if (!this.lifecycleService.willShutdown) {
            this.dialogService.prompt({
                ...
                buttons: [
                    {
                        label: ( localize(13698, "OK")),
                        run: () => { }
                    },
                    {
                        label: ( localize(13699, "Learn More")),
