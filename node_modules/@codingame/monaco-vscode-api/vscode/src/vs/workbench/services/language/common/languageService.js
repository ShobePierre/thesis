...
const languagesExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(14099, 'Contributes language declarations.')),
        type: 'array',
        items: {
            type: 'object',
            ...
            properties: {
                id: {
                    description: ( localize(14100, 'ID of the language.')),
                    type: 'string'
                },
                aliases: {
                    description: ( localize(14101, 'Name aliases for the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                extensions: {
                    description: ( localize(14102, 'File extensions associated to the language.')),
                    default: ['.foo'],
                    type: 'array',
                    items: {
                        type: 'string'
                ...
                filenames: {
                    description: ( localize(14103, 'File names associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                filenamePatterns: {
                    description: ( localize(14104, 'File name glob patterns associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                mimetypes: {
                    description: ( localize(14105, 'Mime types associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                icon: {
                    ...
                    properties: {
                        light: {
                            description: ( localize(14109, 'Icon path when a light theme is used')),
                            type: 'string'
                        },
                        dark: {
                            description: ( localize(14110, 'Icon path when a dark theme is used')),
                            type: 'string'
                        }
                    }
                }
...
class LanguageTableRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [
            ( localize(14111, "ID")),
            ( localize(14112, "Name")),
            ( localize(14113, "File Extensions")),
            ( localize(14114, "Grammar")),
            ( localize(14115, "Snippets"))
        ];
...
( Registry.as(Extensions.ExtensionFeaturesRegistry)).registerExtensionFeature({
    id: 'languages',
    label: ( localize(14116, "Programming Languages")),
    access: {
        canToggle: false
    },
...
function isValidLanguageExtensionPoint(value, collector) {
    if (!value) {
        collector?.error(( localize(14118, "Empty value for `contributes.{0}`", languagesExtPoint.name)));
        return false;
    }
    if (typeof value.id !== 'string') {
        collector?.error(( localize(14119, "property `{0}` is mandatory and must be of type `string`", 'id')));
        return false;
    }
