...
const configurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(14275, 'Contributes extension defined themable colors')),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                id: {
                    type: 'string',
                    description: ( localize(14276, 'The identifier of the themable color')),
                ...
                description: {
                    type: 'string',
                    description: ( localize(14278, 'The description of the themable color')),
                },
                defaults: {
                    type: 'object',
                    properties: {
        ...
        configurationExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(14283, "'configuration.colors' must be a array")));
                    return;
                }
                ...
                for (const colorContribution of extensionValue) {
                    if (typeof colorContribution.id !== 'string' || colorContribution.id.length === 0) {
                        collector.error(( localize(14285, "'configuration.colors.id' must be defined and can not be empty")));
                        return;
                    }
...
class ColorDataRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [
            ( localize(14291, "ID")),
            ( localize(14292, "Description")),
            ( localize(14293, "Dark Default")),
            ( localize(14294, "Light Default")),
            ( localize(14295, "High Contrast Default")),
        ];
