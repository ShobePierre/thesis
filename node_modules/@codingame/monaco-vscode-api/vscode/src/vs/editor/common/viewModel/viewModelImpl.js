...
import { TextDirection, EndOfLinePreference, TrackedRangeStickiness } from '../model.js';
import { InternalModelContentChangeEvent, RawContentChangedType, ModelLineHeightChangedEvent, ModelFontChangedEvent } from '../textModelEvents.js';
...
import { ViewModelEventDispatcher, ScrollChangedEvent, FocusChangedEvent, WidgetFocusChangedEvent, ModelContentChangedEvent, ModelLineHeightChangedEvent as ModelLineHeightChangedEvent$1, ModelFontChangedEvent as ModelFontChangedEvent$1, ModelTokensChangedEvent, ModelLanguageConfigurationChangedEvent, ModelLanguageChangedEvent, ModelOptionsChangedEvent, ModelDecorationsChangedEvent, HiddenAreasChangedEvent, ReadOnlyEditAttemptEvent, ViewZonesChangedEvent as ViewZonesChangedEvent$1 } from '../viewModelEventDispatcher.js';
...
class ViewModel extends Disposable {
    constructor(editorId, configuration, model, domLineBreaksComputerFactory, monospaceLineBreaksComputerFactory, scheduleAtNextAnimationFrame, languageConfigurationService, _themeService, _attachedView, _transactionalTarget) {
        ...
        this.viewLayout = this._register(( new ViewLayout(
            this._configuration,
            this.getLineCount(),
    ...
    _registerModelEvents() {
        ...
        if (allowVariableLineHeights) {
            this._register(this.model.onDidChangeLineHeight((e) => {
                ...
                if (filteredChanges.length > 0) {
                    const filteredEvent = new ModelLineHeightChangedEvent(filteredChanges);
                    this._eventDispatcher.emitOutgoingEvent(( new ModelLineHeightChangedEvent$1(filteredEvent)));
                }
            }));
        }
        ...
        if (allowVariableFonts) {
            this._register(this.model.onDidChangeFont((e) => {
                const filteredChanges = e.changes.filter((change) => change.ownerId === this._editorId || change.ownerId === 0);
                if (filteredChanges.length > 0) {
                    const filteredEvent = new ModelFontChangedEvent(filteredChanges);
                    this._eventDispatcher.emitOutgoingEvent(( new ModelFontChangedEvent$1(filteredEvent)));
                }
            }));
        }
