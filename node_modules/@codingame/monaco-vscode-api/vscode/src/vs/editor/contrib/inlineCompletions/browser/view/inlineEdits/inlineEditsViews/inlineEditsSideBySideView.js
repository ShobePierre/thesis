...
import { maxContentWidthInRange, getContentRenderWidth, observeEditorBoundingClientRect, getOffsetForPos, PathBuilder, mapOutFalsy } from '../utils/utils.js';
...
let InlineEditsSideBySideView = class InlineEditsSideBySideView extends Disposable {
    static fitsInsideViewport(editor, textModel, edit, reader) {
        ...
        const maxOriginalContent = maxContentWidthInRange(editorObs, edit.displayRange, undefined );
    ...
    constructor(_editor, _edit, _previewTextModel, _uiState, _tabAction, _instantiationService, _themeService) {
        ...
        const separatorWidthObs = ( this._uiState.map(
            s => s?.isInDiffEditor ? WIDGET_SEPARATOR_DIFF_EDITOR_WIDTH : WIDGET_SEPARATOR_WIDTH
        ));
        ...
        this._originalOverlay = n.div({
            ...
            const overlayHider = ( layoutInfoObs.map(
                layoutInfo => Rect.fromLeftTopRightBottom(layoutInfo.contentLeft - BORDER_RADIUS - BORDER_WIDTH, layoutInfo.codeRect.top, layoutInfo.contentLeft, layoutInfo.codeRect.bottom + transitionRectSize)
            )).read(reader);
            ...
            const separatorRect = ( overlayRect.map(
                overlayRect => overlayRect.withMargin(separatorWidth, 0, separatorWidth, separatorWidth).intersectHorizontal(intersectionLine)
            ));
            const transitionRect = ( overlayRect.map(
                overlayRect => Rect.fromLeftTopWidthHeight(overlayRect.right - transitionRectSize + BORDER_WIDTH, overlayRect.bottom - BORDER_WIDTH, transitionRectSize, transitionRectSize).intersectHorizontal(intersectionLine)
            ));
            return [
                n.div({
        ...
        this._modifiedOverlay = n.div({
            ...
            const separatorRect = ( overlayRect.map(
                overlayRect => overlayRect.withMargin(separatorWidth, separatorWidth, separatorWidth, 0)
            ));
