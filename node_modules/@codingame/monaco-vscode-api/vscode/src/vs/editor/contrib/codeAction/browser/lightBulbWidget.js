...
let LightBulbWidget = class LightBulbWidget extends Disposable {
    ...
    _updateLightbulbTitle(autoFix, autoRun) {
        ...
        if (autoRun) {
            this.title = ( localize(976, "Run: {0}", this.state.actions.validActions[0].action.title));
        }
            ...
            this.title = ( localize(978, "Show Code Actions ({0})", this._quickFixKbLabel));
        }
        else if (!autoFix) {
            this.title = ( localize(979, "Show Code Actions"));
        }
    }
    set title(value) {
