    ...
    constructor(_editor, _input, _tabAction, instantiationService, _languageService) {
        ...
        this._modifiedOverlay = n.div({
            ...
            const overlayHider = ( overlayLayoutObs.map(
                layoutInfo => Rect.fromLeftTopRightBottom(layoutInfo.contentLeft - BORDER_RADIUS - BORDER_WIDTH, layoutInfo.overlay.top, layoutInfo.contentLeft, layoutInfo.overlay.bottom)
            )).read(reader);
            const separatorWidth = ( this._input.map(
                i => i?.inDiffEditor ? WIDGET_SEPARATOR_DIFF_EDITOR_WIDTH : WIDGET_SEPARATOR_WIDTH
            )).read(reader);
            const overlayRect = ( overlayLayoutObs.map(
                l => l.overlay.withMargin(0, BORDER_WIDTH, 0, l.startsAtContentLeft ? 0 : BORDER_WIDTH).intersectHorizontal(( new OffsetRange(overlayHider.left, Number.MAX_SAFE_INTEGER)))
            ));
