    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === 'invalid') {
                ...
                this.notificationService.warn(( localize(1379, 'Failed to open this link because its target is missing.')));
            }
            else {
                onUnexpectedError(err);
            }
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip
        ? link.tooltip
        : executeCmd
            ? ( localize(1380, 'Execute command'))
            : ( localize(1381, 'Follow link'));
    const kb = useMetaKey
        ? isMacintosh
            ? ( localize(1382, "cmd + click"))
            : ( localize(1383, "ctrl + click"))
        : isMacintosh
            ? ( localize(1384, "option + click"))
            : ( localize(1385, "alt + click"));
    if (link.url) {
        let nativeLabel = '';
        if (/^command:/i.test(( link.url.toString()))) {
            ...
            if (match) {
                ...
                nativeLabel = ( localize(1386, "Execute command {0}", commandId));
            }
        }
...
class OpenLinkAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(1387, "Open Link")),
