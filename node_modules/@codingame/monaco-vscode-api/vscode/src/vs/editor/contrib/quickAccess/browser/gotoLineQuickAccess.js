...
class AbstractGotoLineQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
    ...
    provideWithoutTextEditor(picker) {
        const label = ( localize(1436, "Open a text editor first to go to a line or an offset."));
        picker.items = [{ label }];
    ...
    provideWithTextEditor(context, picker, token) {
        ...
        const updatePickerAndEditor = () => {
            ...
            picker.ariaLabel = ( localize(
                1437,
                "Current position: line {0}, column {1}. {2}",
                cursor.lineNumber,
                cursor.column,
                label
            ));
        ...
        const toggle = ( new Toggle({
            title: ( localize(1438, "Use Zero-Based Offset")),
    ...
    parsePosition(editor, value) {
        ...
        if (!model) {
            return {
                label: ( localize(1436, "Open a text editor first to go to a line or an offset."))
            };
        }
        if (value.startsWith(':')) {
            ...
            if (isNaN(offset)) {
                return {
                    ...
                    label: this.useZeroBasedOffset ?
                        ( localize(1439, "Type a character position to go to (from 0 to {0}).", maxOffset - 1)) :
                        ( localize(1440, "Type a character position to go to (from 1 to {0}).", maxOffset))
                };
            }
            else {
            ...
            if (parts.length < 1 || isNaN(lineNumber)) {
                return {
                    label: ( localize(1442, "Type a line number to go to (from 1 to {0}).", maxLine))
                };
            }
            ...
            return {
                lineNumber,
                column,
                label: ( localize(1441, "Press 'Enter' to go to line {0} at column {1}.", lineNumber, column))
            };
        }
    }
}
