...
const accessibleDiffViewerInsertIcon = registerIcon('diff-review-insert', Codicon.add, ( localize(163, 'Icon for \'Insert\' in accessible diff viewer.')));
const accessibleDiffViewerRemoveIcon = registerIcon('diff-review-remove', Codicon.remove, ( localize(164, 'Icon for \'Remove\' in accessible diff viewer.')));
const accessibleDiffViewerCloseIcon = registerIcon('diff-review-close', Codicon.close, ( localize(165, 'Icon for \'Close\' in accessible diff viewer.')));
...
let View = class View extends Disposable {
    ...
    _render(store) {
        ...
        container.setAttribute('aria-label', ( localize(167, 'Accessible Diff Viewer. Use arrow up and down to navigate.')));
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = (lines) => lines === 0 ? ( localize(168, "no lines changed"))
                    : lines === 1 ? ( localize(169, "1 line changed"))
                        : ( localize(170, "{0} lines changed", lines));
                ...
                cell.appendChild(createTextNode(`${diffIndex + 1}/${diffsLength}: @@ -${r.original.startLineNumber},${r.original.length} +${r.modified.startLineNumber},${r.modified.length} @@`));
    ...
    _createRow(item, lineHeight, width, originalOptions, originalModel, originalModelOpts, modifiedOptions, modifiedModel, modifiedModelOpts) {
        ...
        if (lineContent.length === 0) {
            lineContent = ( localize(172, "blank"));
        }
        let ariaLabel = '';
        switch (item.type) {
            case LineType.Unchanged:
                if (item.originalLineNumber === item.modifiedLineNumber) {
                    ariaLabel = ( localize(173, "{0} unchanged line {1}", lineContent, item.originalLineNumber));
                }
                else {
                    ariaLabel = ( localize(
                        174,
                ...
                break;
            case LineType.Added:
                ariaLabel = ( localize(175, "+ {0} modified line {1}", lineContent, item.modifiedLineNumber));
                break;
            case LineType.Deleted:
                ariaLabel = ( localize(176, "- {0} original line {1}", lineContent, item.originalLineNumber));
                break;
        }
