...
import { isURLDomainTrusted } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/url/common/trustedDomains';
import { readStaticTrustedDomains, configureOpenerTrustedDomainsHandler } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/url/browser/trustedDomains';
    ...
    async validateLink(resource, openOptions) {
        ...
        if (this._trustedDomainService.isValid(resourceUri)) {
            ...
            const { result } = await this._dialogService.prompt({
                ...
                buttons: [
                    {
                        label: ( localize(12866, '&&Open')),
                        run: () => true
                    },
                    {
                        label: ( localize(12867, '&&Copy')),
                        run: () => {
                    ...
                    {
                        label: ( localize(12868, 'Configure &&Trusted Domains')),
                        run: async () => {
