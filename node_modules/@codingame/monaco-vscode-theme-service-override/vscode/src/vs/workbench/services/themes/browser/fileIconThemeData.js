    ...
    processIconThemeDocument(id, iconThemeDocumentLocation, iconThemeDocument) {
        ...
        function collectSelectors(associations, baseThemeClassName) {
            ...
            if (associations) {
                ...
                if (associations.folderExpanded) {
                    addSelector(inline `${qualifier} ${expanded} .folder-icon::before`, associations.folderExpanded);
                    result.hasFolderIcons = true;
                }
                ...
                if (rootFolderExpanded) {
                    addSelector(inline `${qualifier} ${expanded} .rootfolder-icon::before`, rootFolderExpanded);
                    result.hasFolderIcons = true;
                }
                ...
                if (folderNames) {
                    for (const key in folderNames) {
                        ...
                        addSelector(inline `${qualifier} ${selectors.join('')}.folder-icon::before`, folderNames[key]);
                        result.hasFolderIcons = true;
                    }
                }
                ...
                if (folderNamesExpanded) {
                    for (const key in folderNamesExpanded) {
                        ...
                        const name = handleParentFolder(key.toLowerCase(), selectors);
                        selectors.push(inline `.${classSelectorPart(name)}-name-folder-icon`);
                        addSelector(inline `${qualifier} ${expanded} ${selectors.join('')}.folder-icon::before`, folderNamesExpanded[key]);
                        result.hasFolderIcons = true;
                    }
                }
                ...
                if (rootFolderNames) {
                    for (const key in rootFolderNames) {
                        ...
                        addSelector(inline `${qualifier} .${classSelectorPart(name)}-root-name-folder-icon.rootfolder-icon::before`, rootFolderNames[key]);
                        result.hasFolderIcons = true;
                    }
                }
                ...
                if (rootFolderNamesExpanded) {
                    for (const key in rootFolderNamesExpanded) {
                        const name = key.toLowerCase();
                        addSelector(inline `${qualifier} ${expanded} .${classSelectorPart(name)}-root-name-folder-icon.rootfolder-icon::before`, rootFolderNamesExpanded[key]);
                        result.hasFolderIcons = true;
                    }
                }
                ...
                if (languageIds) {
                    ...
                    for (const languageId in languageIds) {
                        addSelector(inline `${qualifier} .${classSelectorPart(languageId)}-lang-file-icon.file-icon::before`, languageIds[languageId]);
                        result.hasFileIcons = true;
                    ...
                    for (const key in fileExtensions) {
                        ...
                        if (segments.length) {
                            for (let i = 0; i < segments.length; i++) {
                                selectors.push(inline `.${classSelectorPart(segments.slice(i).join('.'))}-ext-file-icon`);
                            }
                            selectors.push(inline `.ext-file-icon`);
                        }
                        addSelector(inline `${qualifier} ${selectors.join('')}.file-icon::before`, fileExtensions[key]);
                        result.hasFileIcons = true;
                        hasSpecificFileIcons = true;
                    }
                }
                ...
                if (fileNames) {
                    for (const key in fileNames) {
                        ...
                        addSelector(inline `${qualifier} ${selectors.join('')}.file-icon::before`, fileNames[key]);
                        result.hasFileIcons = true;
                        hasSpecificFileIcons = true;
                    }
                }
        ...
        if (Array.isArray(fonts)) {
            ...
            fonts.forEach(font => {
                ...
                cssRules.push(inline `@font-face { src: ${fontSrcs.join(', ')}; font-family: ${stringValue(font.id)}; font-weight: ${identValue(font.weight)}; font-style: ${identValue(font.style)}; font-display: block; }`);
            ...
            cssRules.push(inline `.show-file-icons .file-icon::before, .show-file-icons .folder-icon::before, .show-file-icons .rootfolder-icon::before { font-family: ${stringValue(fonts[0].id)}; font-size: ${sizeValue(defaultFontSize)}; }`);
        }
        ...
        for (const defId in selectorByDefinitionId) {
            ...
            if (definition) {
                if (definition.iconPath) {
                    cssRules.push(inline `${selectors.join(', ')} { content: ${emQuad}; background-image: ${asCSSUrl(resolvePath(definition.iconPath))}; }`);
                }
            ...
            for (const languageId of this.languageService.getRegisteredLanguageIds()) {
                if (!coveredLanguages[languageId]) {
                    ...
                    if (icon) {
                        ...
                        cssRules.push(inline `${selector} { content: ${emQuad}; background-image: ${asCSSUrl(icon.dark)}; }`);
                        cssRules.push(inline `.vs ${selector} { content: ${emQuad}; background-image: ${asCSSUrl(icon.light)}; }`);
                    }
                }
            }
        }
