...
let TypeHierarchyTreePeekWidget = class TypeHierarchyTreePeekWidget extends PeekViewWidget {
    ...
    async _updatePreview() {
        ...
        const title = this._direction === TypeHierarchyDirection.Supertypes
            ? ( localize(12798, "Supertypes of '{0}'", element.model.root.name))
            : ( localize(12799, "Subtypes of '{0}'", element.model.root.name));
        this.setTitle(title);
    }
    showLoading() {
        this._parent.dataset['state'] = State.Loading;
        this.setTitle(( localize(12800, "Loading...")));
        this._show();
    }
    showMessage(message) {
    ...
    async showModel(model) {
        ...
        if (root.children.length === 0) {
            this.showMessage(this._direction === TypeHierarchyDirection.Supertypes
                ? ( localize(12801, "No supertypes of '{0}'", model.root.name))
                : ( localize(12802, "No subtypes of '{0}'", model.root.name)));
        }
        else {
