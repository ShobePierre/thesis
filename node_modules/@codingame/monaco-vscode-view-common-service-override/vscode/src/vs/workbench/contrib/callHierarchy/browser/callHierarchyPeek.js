...
import { CallHierarchyDirection } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/callHierarchy/common/callHierarchy';
...
import { IdentityProvider, AccessibilityProvider, Sorter, VirtualDelegate, CallRenderer, DataSource } from './callHierarchyTree.js';
...
let CallHierarchyTreePeekWidget = class CallHierarchyTreePeekWidget extends PeekViewWidget {
    ...
    async _updatePreview() {
        ...
        const title = this._direction === CallHierarchyDirection.CallsFrom
            ? ( localize(4536, "Calls from '{0}'", element.model.root.name))
            : ( localize(4537, "Callers of '{0}'", element.model.root.name));
        this.setTitle(title);
    }
    showLoading() {
        this._parent.dataset['state'] = State.Loading;
        this.setTitle(( localize(4538, "Loading...")));
        this._show();
    }
    showMessage(message) {
    ...
    async showModel(model) {
        ...
        if (root.children.length === 0) {
            this.showMessage(this._direction === CallHierarchyDirection.CallsFrom
                ? ( localize(4539, "No calls from '{0}'", model.root.name))
                : ( localize(4540, "No callers of '{0}'", model.root.name)));
        }
        else {
