...
let LanguageStatus = class LanguageStatus {
    ...
    _update() {
        ...
        if (model.combined.length === 0) {
            ...
            const props = {
                name: ( localize(8483, "Editor Language Status")),
                ariaLabel: ( localize(8484, "Editor Language Status: {0}", ariaLabels.join(', next: '))),
    ...
    _renderStatus(container, status, showSeverity, isPinned, store) {
        ...
        const actionLabel = isPinned ? ( localize(8485, "Remove from Status Bar")) : ( localize(8486, "Add to Status Bar"));
    ...
    static _accessibilityInformation(status) {
        ...
        if (status.detail) {
            return { label: ( localize(8487, '{0}, {1}', textValue, status.detail)) };
        }
        else {
            return { label: ( localize(8488, '{0}', textValue)) };
        }
    }
    static _asStatusbarEntry(item) {
        ...
        return {
            name: ( localize(8489, '{0} (Language Status)', item.name)),
...
class ResetAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(8490, "Reset Language Status Interaction Counter")),
