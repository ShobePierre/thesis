...
let ExtensionBisectUi = class ExtensionBisectUi {
    ...
    _showBisectPrompt() {
        const goodPrompt = {
            label: ( localize(13837, "I can't reproduce")),
            run: () => this._commandService.executeCommand('extension.bisect.next', false)
        };
        const badPrompt = {
            label: ( localize(13838, "I can reproduce")),
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(13841, 'Start Extension Bisect')),
    ...
    async run(accessor) {
        ...
        const res = await dialogService.confirm({
            message: ( localize(13842, "Extension Bisect")),
            detail: ( localize(
                13843,
            ...
            primaryButton: ( localize(13844, "&&Start Extension Bisect"))
        });
        if (res.confirmed) {
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(13845, 'Continue Extension Bisect')),
            category: Categories.Help,
            f1: true,
    ...
    async run(accessor, seeingBad) {
        ...
        if (done.bad) {
            await dialogService.info(( localize(13846, "Extension Bisect")), ( localize(
                13847,
            ...
            const res = await dialogService.confirm({
                type: Severity.Info,
                message: ( localize(13846, "Extension Bisect")),
                primaryButton: ( localize(13848, "&&Report Issue & Continue")),
                cancelButton: ( localize(13849, "Continue")),
                detail: ( localize(
                    13850,
                    "Extension Bisect is done and has identified {0} as the extension causing the problem.",
                    done.id
                )),
                checkbox: { label: ( localize(13851, "Keep this extension disabled")), checked: true }
            });
    ...
    async _checkForBad(dialogService, bisectService) {
        const { result } = await dialogService.prompt({
            type: Severity.Info,
            message: ( localize(13852, "Extension Bisect")),
            detail: ( localize(
                13853,
                "Extension Bisect is active and has disabled {0} extensions. Check if you can still reproduce the problem and proceed by selecting from these options.",
            ...
            buttons: [
                {
                    label: ( localize(13854, "I ca&&n't reproduce")),
                    run: () => false
                },
                {
                    label: ( localize(13855, "I can &&reproduce")),
                    run: () => true
                },
                {
                    label: ( localize(13856, "&&Stop Bisect")),
                    run: () => undefined
                }
            ],
            cancelButton: {
                label: ( localize(13857, "&&Cancel Bisect")),
                run: () => null
            }
        });
        return result;
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(13858, 'Stop Extension Bisect')),
            category: Categories.Help,
            f1: true,
            precondition: ExtensionBisectUi.ctxIsBisectActive
        });
