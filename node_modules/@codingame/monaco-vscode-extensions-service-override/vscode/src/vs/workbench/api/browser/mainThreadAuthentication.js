...
let MainThreadAuthentication = class MainThreadAuthentication extends Disposable {
    ...
    $showContinueNotification(message) {
        const yes = ( localize(2453, "Yes"));
        const no = ( localize(2454, "No"));
        const deferredPromise = ( new DeferredPromise());
        let result = false;
    ...
    async loginPrompt(provider, extensionName, recreatingSession, options) {
        ...
        if (options?.learnMore) {
            buttons.push({
                label: ( localize(2458, "Learn more")),
                run: async () => {
    ...
    async continueWithIncorrectAccountPrompt(chosenAccountLabel, requestedAccountLabel) {
        const result = await this.dialogService.prompt({
            message: ( localize(2459, "Incorrect account detected")),
            detail: ( localize(
                2460,
            ...
            buttons: [
                {
                    label: ( localize(2461, 'Keep {0}', chosenAccountLabel)),
                    run: () => chosenAccountLabel
                },
                {
                    label: ( localize(2462, 'Login with {0}', requestedAccountLabel)),
    ...
    async $showDeviceCodeModal(userCode, verificationUri) {
        const { result } = await this.dialogService.prompt({
            type: Severity.Info,
            message: ( localize(2463, "Device Code Authentication")),
            detail: ( localize(
                2464,
            ...
            buttons: [
                {
                    label: ( localize(2465, "Copy & Continue")),
                    run: () => true
                }
            ],
            cancelButton: true
        ...
        if (result) {
            try {
                ...
                this.notificationService.error(( localize(2466, "Failed to open {0}", verificationUri)));
            }
        }
        return false;
    }
    async $promptForClientRegistration(authorizationServerUrl) {
        ...
        const result = await this.dialogService.prompt({
            type: Severity.Info,
            message: ( localize(2467, "Dynamic Client Registration not supported")),
            detail: ( localize(
                2468,
            ...
            buttons: [
                {
                    label: ( localize(2469, "Copy URIs & Proceed")),
                    run: async () => {
                        try {
                            await this.clipboardService.writeText(redirectUrls);
                        }
                        catch (error) {
                            this.notificationService.error(( localize(2470, "Failed to copy redirect URIs to clipboard.")));
                        }
                        return true;
                    }
                },
        ...
        const sharedTitle = ( localize(2472, "Add Client Registration Details"));
        const clientId = await this.quickInputService.input({
            ...
            placeHolder: ( localize(2474, "OAuth client ID (azye39d...)")),
            ignoreFocusLost: true,
            validateInput: async (value) => {
                if (!value || value.trim().length === 0) {
                    return localize(2475, "Client ID is required");
                }
                return undefined;
            }
        });
        ...
        const clientSecret = await this.quickInputService.input({
            ...
            placeHolder: ( localize(2477, "OAuth client secret (wer32o50f...) or leave it blank")),
            password: true,
            ignoreFocusLost: true
        });
        return {
