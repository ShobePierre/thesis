...
import { getTokenClassificationRegistry, typeAndModifierIdPattern } from '@codingame/monaco-vscode-api/vscode/vs/platform/theme/common/tokenClassificationRegistry';
...
const tokenTypeExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(14428, 'Contributes semantic token types.')),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                id: {
                    type: 'string',
                    description: ( localize(14429, 'The identifier of the semantic token type')),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(14430, 'Identifiers should be in the form letterOrDigit[_-letterOrDigit]*')),
                },
                superType: {
                    type: 'string',
                    description: ( localize(14431, 'The super type of the semantic token type')),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(14432, 'Super types should be in the form letterOrDigit[_-letterOrDigit]*')),
                },
                description: {
                    type: 'string',
                    description: ( localize(14433, 'The description of the semantic token type')),
                }
            }
        }
    }
...
const tokenModifierExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(14434, 'Contributes semantic token modifiers.')),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                id: {
                    type: 'string',
                    description: ( localize(14435, 'The identifier of the semantic token modifier')),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(14436, 'Identifiers should be in the form letterOrDigit[_-letterOrDigit]*'))
                },
                description: {
                    type: 'string',
                    description: ( localize(14437, 'The description of the semantic token modifier'))
                }
            }
        }
    }
...
const tokenStyleDefaultsExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(14438, 'Contributes semantic token scope maps.')),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                language: {
                    description: ( localize(14439, 'Lists the languge for which the defaults are.')),
                    type: 'string'
                },
                scopes: {
    ...
    constructor() {
        ...
        tokenTypeExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(14445, "'configuration.semanticTokenType' must be an array")));
                    return;
                }
        ...
        tokenModifierExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                const collector = extension.collector;
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(14446, "'configuration.semanticTokenModifier' must be an array")));
                    return;
                }
                for (const contribution of extensionValue) {
        ...
        tokenStyleDefaultsExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                const collector = extension.collector;
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(14447, "'configuration.semanticTokenScopes' must be an array")));
                    return;
                }
                for (const contribution of extensionValue) {
                    if (contribution.language && typeof contribution.language !== 'string') {
                        collector.error(( localize(14448, "'configuration.semanticTokenScopes.language' must be a string")));
                        continue;
                    }
