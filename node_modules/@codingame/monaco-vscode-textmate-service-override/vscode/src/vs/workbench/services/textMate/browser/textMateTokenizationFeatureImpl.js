...
import { LazyTokenizationSupport, TokenizationRegistry } from '@codingame/monaco-vscode-api/vscode/vs/editor/common/languages';
...
let TextMateTokenizationFeature = class TextMateTokenizationFeature extends Disposable {
    ...
    startDebugMode(printFn, onStop) {
        if (this._debugMode) {
            this._notificationService.error(( localize(14221, "Already Logging.")));
            return;
        }
        ...
        if (this._debugMode) {
            this._progressService.withProgress({
                ...
                buttons: [( localize(14222, "Stop"))]
            }, (progress) => {
                progress.report({
                    message: ( localize(14223, "Preparing to log TM Grammar parsing. Press Stop when finished."))
                });
                return this._getVSCodeOniguruma().then((vscodeOniguruma) => {
                    vscodeOniguruma.setDefaultDebugCall(true);
                    progress.report({
                        message: ( localize(14224, "Now logging TM Grammar parsing. Press Stop when finished."))
                    });
