...
exports.createAnnouncement = async (req, res) => {
  try {
    ...
    try {
      const notificationMessage = `New announcement: ${content.substring(0, 100)}${content.length > 100 ? '...' : ''}`;
      await createNotificationsForSubject(subject_id, notificationMessage, 'announcement');
    } catch (notifErr) {
      console.error('Failed to create notifications:', notifErr);
      // Continue anyway - don't fail the response
    }

    // Broadcast to subscribers of this subject so students see the announcement in real-time
    try {
      realtime.broadcastToSubject(subject_id, { action: 'created', type: 'announcement', announcement: responsePayload.announcement });
    } catch (e) {
      console.error('Failed to broadcast announcement creation:', e && e.message);
    }
...
exports.updateAnnouncement = async (req, res) => {
  try {
    ...
    try {
      realtime.broadcastToSubject(updatedAnnouncement.subject_id, { action: 'updated', type: 'announcement', announcement: updatedAnnouncement });
    } catch (e) {
      console.error('Failed to broadcast announcement update:', e && e.message);
    }
...
exports.deleteAnnouncement = async (req, res) => {
  try {
    ...
    // Verify the announcement belongs to this instructor
    const announcementRows = await runQuery(
      "SELECT announcement_id, subject_id FROM announcements WHERE announcement_id = ? AND instructor_id = ?",
      [announcementId, instructorId]
    );
    ...
    const subjectId = announcementRows[0].subject_id;
    ...
    try {
      realtime.broadcastToSubject(subjectId, { action: 'deleted', type: 'announcement', announcementId });
    } catch (e) {
      console.error('Failed to broadcast announcement deletion:', e && e.message);
    }
